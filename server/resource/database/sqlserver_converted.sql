...
INSERT INTO sys_role_menu(role_id,menu_id) SELECT r.id,m.id FROM sys_role r CROSS JOIN sys_menu m WHERE r.name=N'系统管理员' AND r.deleted_at IS NULL AND m.permission IN (N'gb28181:openapi:client:read',N'gb28181:openapi:client:create',N'gb28181:openapi:client:grant',N'gb28181:openapi:client:rotate',N'gb28181:openapi:client:status',N'gb28181:openapi:client:audit') AND m.deleted_at IS NULL AND NOT EXISTS (SELECT 1 FROM sys_role_menu x WHERE x.role_id=r.id AND x.menu_id=m.id);
...
INSERT INTO sys_casbin_rule(ptype,v0,v1,v2,v3,v4,v5) SELECT DISTINCT N'p',CONCAT(N'role_',rm.role_id),a.path,a.method,N'*',N'',N'' FROM sys_role_menu rm JOIN sys_menu m ON m.id=rm.menu_id JOIN sys_menu_api ma ON ma.menu_id=m.id JOIN sys_api a ON a.id=ma.api_id WHERE rm.role_id IN (SELECT id FROM sys_role WHERE name=N'系统管理员' AND deleted_at IS NULL) AND m.permission LIKE N'gb28181:openapi:client:%' AND m.deleted_at IS NULL AND a.deleted_at IS NULL AND NOT EXISTS (SELECT 1 FROM sys_casbin_rule c WHERE c.ptype=N'p' AND c.v0=CONCAT(N'role_',rm.role_id) AND c.v1=a.path AND c.v2=a.method AND c.v3=N'*');
