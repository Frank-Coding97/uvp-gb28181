-- 播放鉴权配置与独立点播权限（SQL Server，幂等）。
INSERT INTO [sys_api] ([title],[path],[method],[api_group],[created_at],[updated_at],[created_by])
SELECT v.title,v.path,v.method,N'GB28181 SIP 配置',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 (N'读取播放鉴权配置','/api/gb28181/sip/service-config/play-auth','GET'),
 (N'修改播放鉴权配置','/api/gb28181/sip/service-config/play-auth','PUT'),
 (N'发起实时点播','/api/gb28181/play/:deviceId/:channelId','POST'),
 (N'申请固定播放地址授权','/api/gb28181/play/:deviceId/:channelId/authorization','POST')
) v(title,path,method)
...
SELECT 140355,'','',N'发起实时点播',3,'gb28181:play:start',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
...
JOIN [sys_role_menu] rm ON c.[v0]='role_' + CAST(rm.[role_id] AS varchar(20))
