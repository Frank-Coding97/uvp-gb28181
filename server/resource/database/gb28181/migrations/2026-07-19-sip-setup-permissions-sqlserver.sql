-- SIP setup API and administrator permissions (SQL Server, idempotent).

INSERT INTO [sys_api] ([title],[path],[method],[api_group],[created_at],[updated_at],[created_by])
SELECT v.title,v.path,v.method,N'GB28181 SIP 配置',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 (N'读取 SIP 配置状态','/api/gb28181/sip/setup/status','GET'),
 (N'读取本机网络接口','/api/gb28181/sip/setup/network-interfaces','GET'),
 (N'读取 SIP 平台信息','/api/gb28181/sip/platform','GET'),
 (N'保存 SIP 配置','/api/gb28181/sip/setup/config','PUT'),
 (N'暂缓 SIP 配置','/api/gb28181/sip/setup/skip','POST')
) v(title,path,method)
...
FROM (VALUES
 (N'查看 SIP 配置','gb28181:sip:config:view'),
 (N'修改 SIP 配置','gb28181:sip:config:update')
) v(title,permission)
