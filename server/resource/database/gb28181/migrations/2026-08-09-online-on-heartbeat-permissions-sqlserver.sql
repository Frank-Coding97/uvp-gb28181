-- 收到心跳恢复设备上线配置 API 与既有 SIP 配置权限关联（SQL Server，幂等）。

INSERT INTO [sys_api] ([title],[path],[method],[api_group],[created_at],[updated_at],[created_by])
SELECT v.title,v.path,v.method,N'GB28181 SIP 配置',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 (N'读取收到心跳恢复设备上线配置','/api/gb28181/sip/service-config/online-on-heartbeat','GET'),
 (N'修改收到心跳恢复设备上线配置','/api/gb28181/sip/service-config/online-on-heartbeat','PUT')
) v(title,path,method)
...
INSERT INTO [sys_casbin_rule] ([ptype],[v0],[v1],[v2],[v3],[v4],[v5])
SELECT 'p','role_1',a.[path],a.[method],N'*',N'',N'' FROM [sys_api] a
