-- 全局订阅项目和新通道音频默认值 API 权限（SQL Server，幂等）。

INSERT INTO [sys_api] ([title],[path],[method],[api_group],[created_at],[updated_at],[created_by])
SELECT v.title,v.path,v.method,N'GB28181 SIP 配置',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 (N'读取全局订阅项目','/api/gb28181/sip/service-config/global-subscriptions','GET'),
 (N'修改全局订阅项目','/api/gb28181/sip/service-config/global-subscriptions','PUT'),
 (N'读取全局通道音频配置','/api/gb28181/sip/service-config/default-channel-audio','GET'),
 (N'修改全局通道音频配置','/api/gb28181/sip/service-config/default-channel-audio','PUT')
) v(title,path,method)
