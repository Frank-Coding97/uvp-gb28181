-- 忽略通道离线/异常通知配置 API 与既有 SIP 配置权限关联（SQL Server，幂等）。

INSERT INTO [sys_api] ([title],[path],[method],[api_group],[created_at],[updated_at],[created_by])
SELECT v.title,v.path,v.method,N'GB28181 SIP 配置',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 (N'读取忽略通道离线异常通知配置','/api/gb28181/sip/service-config/ignore-channel-offline-status-notify','GET'),
 (N'修改忽略通道离线异常通知配置','/api/gb28181/sip/service-config/ignore-channel-offline-status-notify','PUT')
) v(title,path,method)
