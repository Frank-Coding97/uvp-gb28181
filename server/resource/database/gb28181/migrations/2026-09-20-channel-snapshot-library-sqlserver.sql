...
-- —— SQL Server 方言，与 2026-09-20-channel-snapshot-library.sql 等价。
--
...
WHERE m.permission=N'gb28181:device:snapshot' AND m.type=3 AND m.deleted_at IS NULL
  AND a.path=N'/api/gb28181/device-mgmt/snapshots/:id/content' AND a.method=N'GET' AND a.deleted_at IS NULL
  AND NOT EXISTS (SELECT 1 FROM sys_casbin_rule p WHERE p.ptype='p' AND p.v0=CONCAT('role_',rm.role_id) AND p.v1=N'/api/gb28181/device-mgmt/snapshots/:id/content' AND p.v2=N'GET' AND p.v3='*');
...
WHERE m.permission=N'gb28181:device:snapshot' AND m.type=3 AND m.deleted_at IS NULL
  AND a.path=N'/api/gb28181/device-mgmt/snapshots' AND a.method=N'GET' AND a.deleted_at IS NULL
  AND NOT EXISTS (SELECT 1 FROM sys_casbin_rule p WHERE p.ptype='p' AND p.v0=CONCAT('role_',rm.role_id) AND p.v1=N'/api/gb28181/device-mgmt/snapshots' AND p.v2=N'GET' AND p.v3='*');
