-- SIP 命令超时与预分配模式配置 API 权限（SQL Server，幂等）。

INSERT INTO [sys_api] ([title],[path],[method],[api_group],[created_at],[updated_at],[created_by])
SELECT v.title,v.path,v.method,N'GB28181 SIP 配置',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 (N'读取 SIP 命令超时时间','/api/gb28181/sip/service-config/sip-command-timeout','GET'),
 (N'修改 SIP 命令超时时间','/api/gb28181/sip/service-config/sip-command-timeout','PUT'),
 (N'读取预分配模式','/api/gb28181/sip/service-config/preallocation-mode','GET'),
 (N'修改预分配模式','/api/gb28181/sip/service-config/preallocation-mode','PUT')
) v(title,path,method)
