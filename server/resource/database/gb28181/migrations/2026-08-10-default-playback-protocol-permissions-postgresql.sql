-- 默认播放协议配置 API 与既有 SIP 配置权限关联（PostgreSQL，幂等）。

INSERT INTO sys_api (title,path,method,api_group,created_at,updated_at,created_by)
SELECT v.title,v.path,v.method,'GB28181 SIP 配置',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 ('读取默认播放协议','/api/gb28181/sip/service-config/default-playback-protocol','GET'),
 ('修改默认播放协议','/api/gb28181/sip/service-config/default-playback-protocol','PUT'),
 ('根据字典编码获取字典项列表','/api/sysDictItem/getByDictCode/:dictCode','GET')
