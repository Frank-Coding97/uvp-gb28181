-- Roll back media-management menu/API bindings (mysql).
DELETE c FROM `sys_casbin_rule` c JOIN `sys_api` a ON c.`v1`=a.`path` AND c.`v2`=a.`method` WHERE a.`api_group`='GB28181 媒体管理';
DELETE ma FROM `sys_menu_api` ma JOIN `sys_api` a ON a.`id`=ma.`api_id` WHERE a.`api_group`='GB28181 媒体管理';
...
UPDATE `sys_menu` SET `parent_id`=@media_menu_id,`redirect`='',`title`='流媒体节点',`icon`='lucide:Server',`sort`=10,`updated_at`=NOW()
WHERE `path`='/gb28181/zlm/nodes' AND `deleted_at` IS NULL;
UPDATE `sys_menu` SET `parent_id`=@media_menu_id,`redirect`='',`title`='调度算法',`icon`='lucide:Workflow',`sort`=11,`updated_at`=NOW()
WHERE `path`='/gb28181/zlm/scheduler' AND `deleted_at` IS NULL;
UPDATE `sys_menu` SET `parent_id`=@media_menu_id,`redirect`='',`title`='调度日志',`icon`='lucide:History',`sort`=12,`updated_at`=NOW()
WHERE `path`='/gb28181/zlm/scheduler/logs' AND `deleted_at` IS NULL;
UPDATE `sys_menu` SET `parent_id`=0,`redirect`='',`title`='云端录像',`icon`='lucide:Cloud',`sort`=35,`updated_at`=NOW()
WHERE `path`='/gb28181/cloud-recordings' AND `deleted_at` IS NULL;
UPDATE `sys_menu` SET `parent_id`=0,`redirect`='',`title`='录像计划',`icon`='lucide:CalendarClock',`sort`=34,`updated_at`=NOW()
WHERE `path`='/gb28181/recording-schedules' AND `deleted_at` IS NULL;
UPDATE `sys_menu` SET `redirect`='',`title`='流媒体管理',`updated_at`=NOW() WHERE `path`='/media' AND `deleted_at` IS NULL;
