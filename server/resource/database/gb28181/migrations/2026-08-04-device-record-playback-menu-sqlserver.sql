-- Register device recording playback as a hidden layout route (SQL Server 2017+).
-- The device management menu must already exist.

DECLARE @DEVICE_MGMT_MENU_ID BIGINT = (
    SELECT TOP 1 [id]
    FROM [sys_menu]
...
DECLARE @GB_PARENT_MENU_ID BIGINT = (
    SELECT [parent_id] FROM [sys_menu] WHERE [id] = @DEVICE_MGMT_MENU_ID
);

UPDATE [sys_menu]
SET
    [parent_id] = ISNULL(@GB_PARENT_MENU_ID, [parent_id]),
...
IF @GB_PARENT_MENU_ID IS NOT NULL
   AND NOT EXISTS (
...
DECLARE @PLAYBACK_MENU_ID BIGINT = (
    SELECT MIN([id])
    FROM [sys_menu]
    WHERE [path] = '/gb28181/device-record-playback/:channelId'
      AND [deleted_at] IS NULL
