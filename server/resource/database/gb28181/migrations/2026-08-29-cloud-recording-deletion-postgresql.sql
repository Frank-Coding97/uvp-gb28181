-- Cloud recording physical deletion permissions (PostgreSQL 12+).
INSERT INTO sys_menu (parent_id,path,name,component,title,type,permission,hide,created_at,updated_at,created_by)
SELECT m.id,'','','','删除录像文件',3,'gb28181:recording:delete',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
...
SELECT v.title,v.path,v.method,'GB28181 云端录像删除',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1 FROM (VALUES
  ('删除单个云端录像','/api/gb28181/cloud-recordings/files/:id','DELETE'),
  ('批量删除云端录像','/api/gb28181/cloud-recordings/files/batch-delete','POST')
) v(title,path,method)
