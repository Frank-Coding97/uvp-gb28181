-- Alarm management menu, permissions, APIs and query index (PostgreSQL 12+).
CREATE INDEX IF NOT EXISTS idx_alarm_time ON gb_alarm_event (alarm_time);

UPDATE sys_menu SET name='gb28181-alarm-management',component='gb28181/alarm-management/index',title='告警管理',icon='lucide:BellRing',type=2,permission='gb28181:alarm:view',hide=0,disable=0,updated_at=CURRENT_TIMESTAMP,deleted_at=NULL
...
SELECT dm.parent_id,'/gb28181/alarm-management','gb28181-alarm-management','gb28181/alarm-management/index','告警管理','lucide:BellRing',30,2,'gb28181:alarm:view',0,0,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM sys_menu dm
...
SELECT m.id,'','','','物理删除告警',3,'gb28181:alarm:delete',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
...
SELECT v.title,v.path,v.method,'GB28181 告警管理',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 ('查询告警列表','/api/gb28181/alarms','GET'),
 ('查询告警详情','/api/gb28181/alarms/:id','GET'),
 ('物理删除单条告警','/api/gb28181/alarms/:id','DELETE'),
 ('批量物理删除告警','/api/gb28181/alarms/batch-delete','POST')
) v(title,path,method)
