...
-- Merge cluster and runtime overview into one visible menu (MySQL).
UPDATE `sys_menu` SET `component`='gb28181/zlm/ClusterOverview',`title`='总览',`icon`='lucide:LayoutDashboard',`sort`=10,`hide`=0,`updated_at`=NOW()
WHERE `path`='/gb28181/zlm/overview' AND `deleted_at` IS NULL;
UPDATE `sys_menu` SET `component`='gb28181/zlm/RuntimeOverview',`hide`=1,`updated_at`=NOW()
