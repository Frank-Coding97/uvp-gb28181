-- 设备上线同步通道配置 API 与既有 SIP 配置权限关联（PostgreSQL，幂等）。

INSERT INTO sys_api (title,path,method,api_group,created_at,updated_at,created_by)
SELECT v.title,v.path,v.method,'GB28181 SIP 配置',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES
 ('读取设备上线同步通道配置','/api/gb28181/sip/service-config/sync-channels-on-online','GET'),
 ('修改设备上线同步通道配置','/api/gb28181/sip/service-config/sync-channels-on-online','PUT')
