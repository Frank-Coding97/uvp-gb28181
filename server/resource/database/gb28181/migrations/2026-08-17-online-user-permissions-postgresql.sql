-- 在线用户菜单、API 与权限(PostgreSQL,幂等)。心跳只校验登录态,不进入业务权限。
INSERT INTO sys_api (title,path,method,api_group,created_at,updated_at,created_by)
SELECT v.title,v.path,v.method,'系统管理',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM (VALUES ('查询在线用户','/api/sysOnlineUser/list','GET'),('强制下线会话','/api/sysOnlineUser/forceLogout','POST')) v(title,path,method)
...
SELECT p.id,'/system/online-user','SystemOnlineUser','system/online-user/index','在线用户',0,0,8,2,'system:online-user:list','lucide:UsersRound',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
...
SELECT p.id,'','SystemOnlineUserForceLogout','','强制下线',1,0,1,3,'system:online-user:force-logout',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
FROM sys_menu p WHERE p.path='/system/online-user' AND p.deleted_at IS NULL
