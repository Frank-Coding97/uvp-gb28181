-- Cloud recording download task control APIs (PostgreSQL 13+).
-- The content route is intentionally absent: it is authorized by a one-time HttpOnly cookie.
INSERT INTO sys_api (title,path,method,api_group,created_at,updated_at,created_by)
SELECT v.title,v.path,v.method,'GB28181 云端录像下载',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1 FROM (VALUES
  ('创建云端录像下载','/api/gb28181/cloud-recordings/files/:id/downloads','POST'),
  ('查询云端录像下载','/api/gb28181/cloud-recordings/downloads/:taskId','GET'),
  ('取消云端录像下载','/api/gb28181/cloud-recordings/downloads/:taskId','DELETE')
) v(title,path,method)
WHERE NOT EXISTS (
...
WHERE m.path='/gb28181/cloud-recordings' AND m.deleted_at IS NULL
  AND a.api_group='GB28181 云端录像下载' AND a.deleted_at IS NULL
  AND (
    (a.path='/api/gb28181/cloud-recordings/files/:id/downloads' AND a.method='POST')
    OR (a.path='/api/gb28181/cloud-recordings/downloads/:taskId' AND a.method IN ('GET','DELETE'))
  )
  AND NOT EXISTS (
    ...
    WHERE c.ptype='p' AND c.v0='role_' || rm.role_id::text
