-- Refuse destructive rollback while dashboard state or metric facts exist.
DELIMITER $$
CREATE PROCEDURE uvp_drop_home_dashboard_if_empty()
BEGIN
  IF EXISTS (SELECT 1 FROM gb_dashboard_layout LIMIT 1)
    ...
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'home dashboard tables are not empty';
  END IF;
...
END$$
DELIMITER ;
CALL uvp_drop_home_dashboard_if_empty();
DROP PROCEDURE uvp_drop_home_dashboard_if_empty;
