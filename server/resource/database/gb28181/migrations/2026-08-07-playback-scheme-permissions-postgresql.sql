-- Playback scheme APIs and one hidden permission point. PostgreSQL.
INSERT INTO sys_api (title,path,method,api_group,created_at,updated_at,created_by)
SELECT v.title,v.path,v.method,'GB28181 多屏播放',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1 FROM (VALUES
 ('查询播放方案','/api/gb28181/playback-schemes','GET'),
 ('查看播放方案','/api/gb28181/playback-schemes/:id','GET'),
 ('创建播放方案','/api/gb28181/playback-schemes','POST'),
 ('重命名播放方案','/api/gb28181/playback-schemes/:id','PATCH'),
 ('覆盖播放方案','/api/gb28181/playback-schemes/:id/layout','PUT'),
 ('删除播放方案','/api/gb28181/playback-schemes/:id','DELETE')) v(title,path,method)
WHERE NOT EXISTS (SELECT 1 FROM sys_api a WHERE a.path=v.path AND a.method=v.method AND a.deleted_at IS NULL);
INSERT INTO sys_menu (parent_id,path,name,title,type,permission,hide,created_at,updated_at,created_by) SELECT 140355,'','','管理播放方案',3,'gb28181:playback-scheme:manage',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1 WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE permission='gb28181:playback-scheme:manage' AND deleted_at IS NULL);
