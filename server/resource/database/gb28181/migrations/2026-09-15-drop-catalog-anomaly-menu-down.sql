...
-- 恢复「目录异常」菜单行与角色绑定(物理删除的逆操作,MySQL)。
-- 注意:前端页面 web/src/views/gb28181/device-mgmt/anomaly/index.vue 已随本次清理
-- 一并删除,回滚后需从 git 历史恢复该文件,菜单才可正常打开。
...
SELECT 140357,0,'/gb28181/device-mgmt/anomaly','device-mgmt-anomaly','gb28181/device-mgmt/anomaly/index','目录异常',6,2,1,0,0,'lucide:Cctv',0,NOW(),NOW()
FROM DUAL
