...
-- Physically drop the duplicate "GB28181 access security" menu (SQL Server, idempotent).
...
DELETE FROM [sys_menu_api] WHERE [menu_id] IN (SELECT [id] FROM [sys_menu] WHERE [path]=N'/gb28181/security');
DELETE FROM [sys_role_menu] WHERE [menu_id] IN (SELECT [id] FROM [sys_menu] WHERE [path]=N'/gb28181/security');
DELETE FROM [sys_menu] WHERE [path]=N'/gb28181/security';
