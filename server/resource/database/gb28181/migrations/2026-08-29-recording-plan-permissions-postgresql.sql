-- Recording plan menus and API permissions (PostgreSQL, idempotent).
INSERT INTO sys_menu (parent_id,path,name,component,title,type,permission,icon,sort,created_at,updated_at,created_by)
SELECT 0,'/gb28181/recording-schedules','gb28181-recording-schedules','gb28181/recording-schedules/index','录像计划',2,'gb28181:recording-plan:view','lucide:CalendarClock',34,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1
...
SELECT p.id,'','','','维护录像计划',3,'gb28181:recording-plan:maintain',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1 FROM sys_menu p
...
INSERT INTO sys_menu (parent_id,path,name,component,title,type,permission,hide,created_at,updated_at,created_by)
SELECT p.id,'','','','分配录像计划',3,'gb28181:recording-plan:assign',1,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1 FROM sys_menu p
...
SELECT s.title,s.path,s.method,'GB28181 录像计划',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,1 FROM (VALUES
 ('查询录像计划','/api/gb28181/recording-plans','GET'),('新建录像计划','/api/gb28181/recording-plans','POST'),
 ('查看录像计划','/api/gb28181/recording-plans/:id','GET'),('编辑录像计划','/api/gb28181/recording-plans/:id','PUT'),
 ('删除录像计划','/api/gb28181/recording-plans/:id','DELETE'),('启停录像计划','/api/gb28181/recording-plans/:id/status','PATCH'),
...
SELECT DISTINCT 'p','role_'||rm.role_id,a.path,a.method,'*','','' FROM sys_role_menu rm JOIN sys_menu m ON m.id=rm.menu_id JOIN sys_menu_api ma ON ma.menu_id=m.id JOIN sys_api a ON a.id=ma.api_id
