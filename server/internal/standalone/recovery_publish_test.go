...
type recoveryPublicationFixture struct {
	paths   Paths
	owner   *upgradePreparation
	outer   MaintenanceJournal
	journal recoveryJournal
	sealed  recoveryJournal
	trust   string
}

func newRecoveryPublicationFixture(t *testing.T) recoveryPublicationFixture {
	t.Helper()
	...
	return recoveryPublicationFixture{paths: paths, owner: owner, outer: outer, journal: j, sealed: next, trust: trust}
}
...
func TestRecoveryPublicationResumesEachUnrecordedRename(t *testing.T) {
	for _, stop := range []string{"none", "config_isolated", "config_published", "data_isolated", "data_published"} {
		t.Run(stop, func(t *testing.T) {
			...
			paths, outer, j, next, trust := fixture.paths, fixture.outer, fixture.journal, fixture.sealed, fixture.trust
			failed := filepath.Join(paths.InstallDir, maintenanceDirName, "failed", j.OperationID)
			if stop == "none" {
