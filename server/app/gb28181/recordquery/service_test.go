...
func TestServiceTerminalStatesAndCapacity(t *testing.T) {
	tests := []struct {
		...
		{name: "empty with explicit empty list", respond: func(s *Service, sender string, sn int) {
			s.Accept(sender, &manscdp.RecordInfoResponse{SN: sn, DeviceID: "channel-a", SumNum: 0, RecordListPresent: true})
		}, wantStatus: QueryStatusEmpty},
