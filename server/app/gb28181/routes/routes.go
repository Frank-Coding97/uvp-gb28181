...
func RegisterRoutes(protected *gin.RouterGroup) {
	gb := protected.Group("/gb28181")
	{
		home := gb.Group("/home")
		{
			...
			home.GET("/drilldown/play", homeDashboardController.PlayHistory)
			home.GET("/drilldown/traffic", homeDashboardController.TrafficHistory)
		...
		zlm := gb.Group("/zlm")
		{
			...
			zlm.POST("/nodes/:id/enable", zlmNodeRoute(func(ctrl *gbcontrollers.ZLMNodeController, c *gin.Context) { ctrl.Enable(c) }))
			zlm.POST("/nodes/:id/disable", zlmNodeRoute(func(ctrl *gbcontrollers.ZLMNodeController, c *gin.Context) { ctrl.Disable(c) }))
