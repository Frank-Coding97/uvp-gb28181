...
func TestDeviceOperationRTPStepsMigrationPreservesUnknownHistory(t *testing.T) {
	for _, target := range []struct{ suffix, baseline string }{
		...
		t.Run(target.baseline, func(t *testing.T) {
			stem := "migrations/2026-09-07-device-operation-rtp-steps" + target.suffix
			...
			for _, word := range []string{"gb_device_operation_intent", "rtp_steps_json", "null", "ck_device_intent_rtp_size", "32768"} {
			...
			begin, end := "-- device-operation-rtp-steps:begin", "-- device-operation-rtp-steps:end"
