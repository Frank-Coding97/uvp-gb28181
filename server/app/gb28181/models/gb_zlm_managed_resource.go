...
//
// 这张表只保留来源审计所需的安全摘要和完整媒体身份。ZLM 的实时列表才是
// 资源是否存在的事实来源；因此模型没有 online/status、desired state 或原始响应字段。
...
type GbZLMManagedResource struct {
	...
	Schema              string     `gorm:"column:schema;size:32;not null;default:'';uniqueIndex:uk_gb_zlm_managed_resource_identity,priority:4" json:"schema"`
	Vhost               string     `gorm:"column:vhost;size:128;not null;default:'';uniqueIndex:uk_gb_zlm_managed_resource_identity,priority:5" json:"vhost"`
	App                 string     `gorm:"column:app;size:64;not null;default:'';uniqueIndex:uk_gb_zlm_managed_resource_identity,priority:6" json:"app"`
	Stream              string     `gorm:"column:stream;size:255;not null;default:'';uniqueIndex:uk_gb_zlm_managed_resource_identity,priority:7" json:"stream"`
