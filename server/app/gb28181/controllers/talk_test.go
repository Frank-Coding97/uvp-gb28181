...
func (f *fakeTalkSessionService) Create(_ context.Context, request talk.CreateRequest) (*talk.CreateResult, error) {
	f.createCalls++
	f.request = request
	return &talk.CreateResult{SessionID: "talk-session", State: "reserved"}, nil
}
...
func TestTalkControllerCreateScopesChannelAndStoresActor(t *testing.T) {
	...
	router.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/channel/"+strconv.FormatUint(uint64(ownID), 10)+"/talk-sessions", nil))
	...
	router.ServeHTTP(hidden, httptest.NewRequest(http.MethodPost, "/channel/"+strconv.FormatUint(uint64(hiddenID), 10)+"/talk-sessions", nil))
	require.Equal(t, 1, service.createCalls)
}
