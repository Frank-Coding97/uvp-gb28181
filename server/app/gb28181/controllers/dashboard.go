...
type DashboardSnapshotProvider interface {
	Snapshot(context.Context, time.Duration, time.Duration) (*metrics.DashboardSnapshot, error)
}
...
type DashboardController struct {
	...
	snapshotProvider DashboardSnapshotProvider
}

// NewDashboardController 创建看板控制器
func NewDashboardController(p AggregatorProvider, snapshots ...DashboardSnapshotProvider) *DashboardController {
	controller := &DashboardController{provider: p}
	if len(snapshots) > 0 {
		controller.snapshotProvider = snapshots[0]
	}
	return controller
}
...
func (dc *DashboardController) Snapshot(c *gin.Context) {
	...
	snapshot, err := dc.currentSnapshot(c.Request.Context(), win, prec)
	if err != nil {
		dc.Fail(c, "SIP 协议监控快照读取失败", err, http.StatusServiceUnavailable)
		return
	}
	dc.Success(c, snapshot)
}
...
func (dc *DashboardController) Stream(c *gin.Context) {
	win, prec := parsePulseParams(c)
	first, err := dc.currentSnapshot(c.Request.Context(), win, prec)
	if err != nil {
		dc.Fail(c, "SIP 协议监控快照读取失败", err, http.StatusServiceUnavailable)
		return
	}
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-tick.C:
			snapshot, snapshotErr := dc.currentSnapshot(ctx, win, prec)
			if snapshotErr != nil || !writeSnapshotEvent(c, snapshot) {
				return
			}
			c.Writer.Flush()
		case <-ping.C:
...
// currentSnapshot 优先读取持久化统计，未装配时兼容原内存聚合器。
func (dc *DashboardController) currentSnapshot(ctx context.Context, win, prec time.Duration) (*metrics.DashboardSnapshot, error) {
	if dc.snapshotProvider != nil {
		return dc.snapshotProvider.Snapshot(ctx, win, prec)
	}
	if dc.provider == nil || dc.provider() == nil {
		return emptySnapshot(), nil
	}
	return dc.provider().Snapshot(win, prec), nil
}

// writeSnapshotEvent 写一帧 event: snapshot,返回 false 表示连接已断
func writeSnapshotEvent(c *gin.Context, snapshot *metrics.DashboardSnapshot) bool {
