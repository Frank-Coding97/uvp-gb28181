...
func TestWorkRecordingControllerExplainsContinuousRecordingConflict(t *testing.T) {
	db := newScopedDeviceDB(t)
	seedDeptScopedUser(t, db, 100, 10)
	require.NoError(t, db.AutoMigrate(&models.GbRecorderClaim{}))
	channel := models.GbChannel{DeviceID: "ours", ChannelID: "own", OwnerDeptID: 10, CloudRecordingEnabled: true}
	require.NoError(t, db.Create(&channel).Error)
	require.NoError(t, db.Create(&models.GbRecorderClaim{ResourceKey: workrecording.ChannelResource(channel.ID), ChannelID: channel.ID, OwnerKind: workrecording.OwnerLegacy, OwnerID: "channel:1", State: workrecording.StateUnknown, Version: 1}).Error)
	response := workRequest(workRecordingRouter(t, db, &workRecordingAPIFake{}, 100), http.MethodGet, "/work-recordings/status?channelIds="+uintStr(channel.ID), "")
	require.Equal(t, http.StatusOK, response.Code, response.Body.String())
	require.Contains(t, response.Body.String(), "该通道已启用云端连续录像，请先关闭云端录像再开始作业录像")
}

func TestWorkRecordingControllerDownloadsSingleCameraBatchAsZIP(t *testing.T) {
	db := newScopedDeviceDB(t)
	seedDeptScopedUser(t, db, 100, 10)
	...
	channel := models.GbChannel{DeviceID: "ours", ChannelID: "own", OwnerDeptID: 10}
	require.NoError(t, db.Create(&channel).Error)
	batch := models.GbWorkRecordingBatch{ID: "9b057ecd-215f-4caf-af40-6ca570ff1f62", CreatedBy: 100, RequestID: "single-camera", State: workrecording.StateStopped, Version: 1, FormState: workrecording.FormDraft, SchemaVersion: 1, FormJSON: "{}"}
	require.NoError(t, db.Create(&batch).Error)
	root := t.TempDir()
	path := filepath.Join(root, "slice-1.mp4")
	require.NoError(t, os.WriteFile(path, []byte("video"), 0o600))
	job := models.GbWorkRecording{ID: "9710bd64-e63a-4d34-b2bd-7b70547f1215", BatchID: batch.ID, ChannelID: channel.ID, CreatedBy: 100, RequestID: "single-camera/1", State: workrecording.StateStopped, DesiredAction: workrecording.DesiredActionStop, Version: 1, RecordingRoot: root, FileState: workrecording.FileReady, FormState: workrecording.FormDraft, SchemaVersion: 1, FormJSON: "{}"}
	require.NoError(t, db.Create(&job).Error)
	file := models.GbRecordingFile{ChannelID: channel.ID, DeviceID: channel.DeviceID, NodeID: 1, VHost: "__defaultVhost__", App: "rtp", Stream: "stream", FileKey: "single-camera-file", FileName: "slice-1.mp4", FilePath: path, MetadataState: models.RecordingMetadataComplete}
	...
	router.GET("/work-recordings/batches/:batchId/download", controller.BatchDownload)
	response := workRequest(router, http.MethodGet, "/work-recordings/batches/"+batch.ID+"/download", "")
	require.Equal(t, http.StatusOK, response.Code, response.Body.String())
	...
	require.NoError(t, err)
	require.Equal(t, []byte("video"), payload)
}
