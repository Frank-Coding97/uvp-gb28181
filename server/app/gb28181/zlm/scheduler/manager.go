...
func (m *Manager) Pick(ctx context.Context, inv InviteContext) (*node.Node, error) {
	...
	if inv.PreferredNodeID > 0 && m.factory != nil && m.factory.reg != nil {
		if preferred, ok := m.factory.reg.Get(inv.PreferredNodeID); ok && preferred.IsSchedulable() && !preferred.IsNearCapacity() && !m.factory.reg.IsAdmissionBlocked(preferred.ID) {
			if log != nil {
