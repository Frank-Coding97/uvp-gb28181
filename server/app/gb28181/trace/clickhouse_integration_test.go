...
func TestClickHouseIntegrationWriteListReadDelete(t *testing.T) {
	...
	responseEventID := uuid.NewString()
	...
	responseRaw := []byte("SIP/2.0 200 OK\r\n" +
		"From: <sip:34020000001320000001@3402000000>;tag=1\r\n" +
		"To: <sip:34020000002000000001@3402000000>;tag=2\r\n" +
		"Call-ID: " + callID + "\r\nCSeq: 9 MESSAGE\r\nContent-Length: 0\r\n\r\n")
	responsePayload, err := cipher.Encrypt(responseRaw)
	require.NoError(t, err)
	responseEvent := StoredEvent{
		EventID: responseEventID, OccurredAt: now.Add(time.Millisecond), Direction: DirectionOutbound, Transport: "UDP",
		LocalAddr: "192.168.10.220:5061", RemoteAddr: "192.168.10.10:5060",
		DeviceID: "34020000001320000001", Method: "MESSAGE", StatusCode: 200, CallID: callID,
		CSeq: 9, CSeqMethod: "MESSAGE", Payload: responsePayload,
	}
	require.NoError(t, store.InsertBatch(ctx, []StoredEvent{event, responseEvent}))
	defer func() {
		cleanupCtx, cleanupCancel := context.WithTimeout(context.Background(), 10*time.Second)
		defer cleanupCancel()
		require.NoError(t, store.conn.Exec(cleanupCtx,
			fmt.Sprintf("ALTER TABLE %s DELETE WHERE call_id = ? SETTINGS mutations_sync = 1", store.fullTable),
			callID,
		))
		require.NoError(t, store.conn.Exec(cleanupCtx,
			fmt.Sprintf("ALTER TABLE %s.sip_trace_session_day DELETE WHERE call_id = ? SETTINGS mutations_sync = 1", store.database),
			callID,
		))
	}()
	...
	require.Equal(t, callID, page.Items[0].CallID)
	detail, err := store.GetMessage(ctx, eventID)
	require.NoError(t, err)
	decrypted, err := cipher.Decrypt(detail.Payload)
	require.NoError(t, err)
	require.Equal(t, raw, decrypted)
	sessions, err := store.ListSessions(ctx, SessionFilter{From: now.Add(-time.Minute), To: now.Add(time.Minute), CallID: callID, Limit: 10})
	require.NoError(t, err)
	require.Len(t, sessions, 1)
	require.EqualValues(t, 2, sessions[0].MessageCount)
	require.EqualValues(t, 1, sessions[0].InboundCount)
	require.EqualValues(t, 1, sessions[0].OutboundCount)
	require.EqualValues(t, 200, sessions[0].FinalStatus)
	require.False(t, sessions[0].MissingResponse)
	require.False(t, sessions[0].Anomaly)
	require.True(t, sessions[0].OriginalAvailable)
}
