...
func (sc *SysApiController) List(c *gin.Context) {
	...
	// 获取总数
	count, err := sysApiList.GetCount(c.Request.Context(), req.Handler())
	if err != nil {
		sc.FailAndAbort(c, "统计API数量失败", err)
	}

	err = sysApiList.Find(c.Request.Context(), req.Paginate(), req.Handler(), func(d *gorm.DB) *gorm.DB {
...
func (sc *SysApiController) GetByID(c *gin.Context) {
	...
	err = api.Find(c.Request.Context(), func(d *gorm.DB) *gorm.DB {
...
func (sc *SysApiController) Add(c *gin.Context) {
	...
	err = app.DBContext(c.Request.Context()).Create(api).Error
	if err != nil {
...
func (sc *SysApiController) Update(c *gin.Context) {
	...
	err = app.DBContext(c.Request.Context()).Save(api).Error
	if err != nil {
		sc.FailAndAbort(c, "更新API失败", err)
	}
	err = sc.CasbinService.UpdateRoleApiPermissionsByApiID(c.Request.Context(), req.ID)
	if err != nil {
...
func (sc *SysApiController) Delete(c *gin.Context) {
	...
	// 检查API是否与菜单有关联
	var menuCount int64
	err = app.DBContext(c.Request.Context()).Model(&models.SysMenuApi{}).Where("api_id = ?", req.ID).Count(&menuCount).Error
	if err != nil {
	...
	// 软删除API
	err = app.DBContext(c.Request.Context()).Where("id = ?", req.ID).Delete(api).Error
	if err != nil {
	...
	err = app.DBContext(c.Request.Context()).Where("api_id = ?", req.ID).Delete(&models.SysMenuApi{}).Error
	if err != nil {
