...
func TestSyncChannelsOnOnlinePermissionMigrations(t *testing.T) {
	_, currentFile, _, ok := runtime.Caller(0)
	require.True(t, ok)
	serverRoot := filepath.Clean(filepath.Join(filepath.Dir(currentFile), "..", ".."))

	for _, filename := range []string{
		"2026-08-09-sync-channels-on-online-permissions.sql",
		"2026-08-09-sync-channels-on-online-permissions-postgresql.sql",
		"2026-08-09-sync-channels-on-online-permissions-sqlserver.sql",
	} {
