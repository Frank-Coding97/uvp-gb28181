...
// WithTransportLayerWriteObserver observes final serialized outbound bytes.
// Observer panics are recovered and cannot change transport write results.
...
func (l *TransportLayer) withTransports(conf TransportsConfig) {
	if conf.UDP != nil && l.udp == nil {
		...
		l.udp.writeObserver = l.writeObserver
	}
