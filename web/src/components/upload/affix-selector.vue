<template>
  ...
  >
    <div class="affix-selector">
      <s-layout-search>
        <template #fields>
          <a-input v-model="form.name" placeholder="请输入图片名称" style="width: 220px" allow-clear @press-enter="search" />
        </template>
        <template #actions>
      ...
      >
        <template #columns>
          <a-table-column title="图片名称" data-index="name" :ellipsis="true" tooltip :width="200"></a-table-column>
      ...
      <!-- 图片预览模态框 -->
      <a-modal v-model:visible="previewVisible" :footer="false" :width="600" title="图片预览">
...
const tableScroll = computed(() => ({
  x: "100%",
  ...(affixList.value.length > 0 ? { y: 400 } : {})
}));
