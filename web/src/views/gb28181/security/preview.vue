...
import {
  ...
  formatShortWindowInviteRule,
  isHighRiskSecurityReason
} from "./securityFormatting";
...
const blackRulePageSize = ref(10);
const blackRuleTotal = ref(0);
const allowRulePage = ref(1);
const allowRulePageSize = ref(10);
...
const filteredEvents = computed(() =>
  events.value.filter(item => {
    ...
    const keywordMatches =
      !keyword || [item.source, item.method, item.userAgent, item.rule].some(value => value.toLowerCase().includes(keyword));
...
const filteredAutoBans = computed(() => {
  ...
  return autoBans.value.filter(item => {
    const keywordMatches =
      !keyword ||
      [item.source, item.location, item.reason, item.evidence, item.mode, item.method].some(value =>
...
const filteredCurrentRules = computed(() => {
  ...
  return currentRules.value.filter(item => {
    const keywordMatches =
      !keyword || [item.value, item.note, item.scope, item.type].some(value => value.toLowerCase().includes(keyword));
...
const banPagination = computed(() => ({
  total: banTotal.value,
  current: banPage.value,
  pageSize: banPageSize.value,
  showTotal: true,
  showJumper: true,
  showPageSize: true,
  pageSizeOptions: tablePageSizeOptions
...
const rulePagination = computed(() => ({
  total: currentRuleTotal.value,
  current: currentRulePage.value,
  pageSize: currentRulePageSize.value,
  showTotal: true,
  showJumper: true,
  showPageSize: true,
  pageSizeOptions: tablePageSizeOptions
...
const recognizedCount = computed(() =>
  securityEvents.value.reduce((total, item) => total + Math.max(0, Number(item.count) || 0), 0)
);
const blockedCount = computed(() =>
  securityEvents.value.reduce(
    (total, item) => total + (["drop", "ban"].includes(item.action) ? Math.max(0, Number(item.count) || 0) : 0),
    0
  )
);
const overviewEvents = computed(() =>
  (securitySnapshot.value?.events || [])
    .slice()
    .sort((left, right) => Date.parse(right.lastSeenAt) - Date.parse(left.lastSeenAt))
    .map((item, index) => mapEvent(item, index))
);
const overviewBans = computed(() =>
  (securitySnapshot.value?.bans || [])
    .filter(item => item.status === "active" || item.status === "agent_failed")
    .map((item, index) => mapBan(item, index))
);
...
const shortWindowInviteRuleLabel = computed(() =>
  formatShortWindowInviteRule(securityPolicy.value?.window, securityPolicy.value?.banScore)
);
const lowFrequencyInviteRuleLabel = "10 分钟累计 10 次未授权 INVITE";
const sourceVerificationNote =
  "发现非法 ID 枚举只代表高危并拒绝，不直接永久封 IP；自动永久 IP 封禁需来源可验证（真实 TCP 或平台回程验证），并排除有成功认证记录的共享出口。单向 UDP 未验证来源只拒绝并记录，INVITE 自动 IP 封禁同样适用。";
const automaticBanTTLLabel = computed(() =>
  formatAutomaticBanTTL(
    securityPolicy.value?.banTTLs || [],
    securityPolicy.value?.permanentAutoBan === true,
    securityPolicy.value?.banScore
  )
);
const liveStatus = computed(() => {
  ...
  return {
    label: securityAgent.value.connected ? "防护运行中" : "应用层防护运行中",
    color: securityAgent.value.connected ? ("green" as const) : ("orange" as const)
  };
});
const agentStatus = computed(() => {
  ...
  return {
    title: "主机防火墙未连接",
    detail: securityAgent.value.lastError
      ? `${securityAgent.value.lastError} · 应用层拦截仍生效`
      : "尚未收到 Agent 心跳 · 应用层拦截仍生效",
    label: "降级",
    color: "orange"
  };
});
...
function buildTrendChartSpec() {
  ...
  return {
    ...
    axes: [
      {
        orient: "left",
        grid: { visible: true, style: { stroke: border, lineDash: [3, 4] } },
        label: { style: { fill: tertiary, fontSize: 11 } },
    ...
    tooltip: {
      mark: {
        content: [{ key: (datum: { series: string }) => datum.series, value: (datum: { value: number }) => `${datum.value} 次` }]
      }
    },
...
function mapRule(rule: SecurityAccessRule): AccessRule {
  return {
    ...
    type: uiRuleType(rule.matchType),
    value: rule.matchValue,
    note: rule.note,
    scope: rule.scope,
    expires: rule.expiresAt ? new Date(rule.expiresAt).toLocaleString() : "永久",
...
function mapEvent(event: SecurityEventAggregate, index: number, offset = 0): SecurityEvent {
  ...
  return {
    id: offset + index + 1,
    severity: highRisk ? "高危" : event.action === "drop" ? "中危" : "低危",
    source: event.sourceIp,
    location,
    method: event.method || "未知",
    userAgent: event.userAgent || "未上报",
    rule: formatSecurityReason(event.reason),
    action: formatSecurityAction(event.action),
    count: event.count,
    time: new Date(event.lastSeenAt).toLocaleString()
  };
}
...
function mapBan(ban: FirewallBan, index: number, offset = 0): AutoBanRecord {
  ...
  return {
    ...
    source: decision.sourceIp,
    location,
    method: decision.triggerMethod || "未知",
    reason: formatSecurityReason(decision.reason),
    evidence: `${decision.windowSeconds || 0} 秒内 ${decision.triggerCount || 0} 次，阈值 ${decision.triggerThreshold || decision.score}，累计评分 ${decision.score}`,
    mode: decision.policyMode === "strict" ? "严格" : "保护",
    firewallState: applied ? "已生效" : ban.agentState === "failed" ? "同步失败" : "待同步",
    createdAt: new Date(decision.createdAt).toLocaleTimeString(),
    expires: formatRemaining(decision.createdAt, decision.ttl, Date.now(), decision.permanent === true),
    blocked: ban.blockedCountAfterBan || 0
  };
}
...
async function refreshPreview(showMessage = false, forceLists = false) {
  ...
  try {
    ...
    const [snapshot, eventResult, banResult, policy, black, allow, agentHealth] = await Promise.all([
      ...
      shouldRefreshBans
        ? listSecurityBans({ page: banPage.value, pageSize: banPageSize.value, activeOnly: true })
        : Promise.resolve(null),
      getSecurityPolicy(),
      shouldRefreshBlackRules
        ? listSecurityAccessRules("blacklist", { page: blackRulePage.value, pageSize: blackRulePageSize.value })
        : Promise.resolve(null),
      shouldRefreshAllowRules
        ? listSecurityAccessRules("allowlist", { page: allowRulePage.value, pageSize: allowRulePageSize.value })
        : Promise.resolve(null),
      getSecurityAgentHealth()
    ]);
    ...
    if (eventResult?.code === 0) {
      const items = (eventResult.data?.items || [])
        .slice()
        .sort((left, right) => Date.parse(right.lastSeenAt) - Date.parse(left.lastSeenAt));
    ...
    if (banResult?.code === 0) {
      ...
      autoBans.value = (banResult.data?.items || []).map((item, index) =>
        mapBan(item, index, (banPage.value - 1) * banPageSize.value)
      );
    }
    if (policy.code === 0 && policy.data) {
      securityPolicy.value = policy.data;
      selectedMode.value = policy.data.mode;
      maxUdpThreshold.value = policy.data.maxUdpPerWindow;
      banThreshold.value = policy.data.banScore;
    }
    if (black?.code === 0) {
      blackRuleTotal.value = black.data?.total || 0;
      blackRules.value = (black.data?.items || []).map(mapRule);
    }
    if (allow?.code === 0) {
      allowRuleTotal.value = allow.data?.total || 0;
      allowRules.value = (allow.data?.items || []).map(mapRule);
    }
...
function handleBanPageChange(page: number) {
  banPage.value = page;
  void refreshPreview(false, true);
}
function handleBanPageSizeChange(pageSize: number) {
  banPageSize.value = pageSize;
  banPage.value = 1;
  void refreshPreview(false, true);
}
function resetEventSearch() {
  eventSeverity.value = "全部风险";
  eventSearch.value = "";
}
function resetBanSearch() {
  banSearch.value = "";
  banFirewallState.value = "全部状态";
}
function resetRuleSearch() {
  ruleSearch.value = "";
  ruleTypeFilter.value = "全部类型";
  ruleStatusFilter.value = "全部状态";
}
...
function handleRulePageSizeChange(pageSize: number) {
  if (activeTab.value === "allowlist") {
    allowRulePageSize.value = pageSize;
    allowRulePage.value = 1;
  } else {
    blackRulePageSize.value = pageSize;
    blackRulePage.value = 1;
  }
  void refreshPreview(false, true);
}
...
async function saveRule() {
  ...
  const result = await createSecurityAccessRule({
    listType: editingRuleKind.value === "blacklist" ? "blacklist" : "allowlist",
    matchType: type,
    matchValue: ruleValue.value.trim(),
    scope: ruleType.value === "User-Agent" ? "public" : "all_sip",
    status: "enabled",
    expiresAt: expiryToIso(ruleExpiry.value),
    note: ruleNote.value.trim() || "手动添加"
  });
  if (result.code !== 0) {
    Message.error(result.message || "规则保存失败");
    return;
  }
...
async function removeRule(_kind: RuleKind, id: number) {
  ...
  if (result.code !== 0) {
    Message.error(result.message || "规则删除失败");
    return;
  }
  await refreshPreview(false, true);
...
async function unbanPreview(item: AutoBanRecord) {
  ...
  if (result.code !== 0) {
    Message.error(result.message || "解除封禁失败");
    return;
  }
  await refreshPreview(false, true);
...
async function promoteToBlacklist(item: AutoBanRecord) {
  if (!blackRules.value.some(rule => rule.type === "IP" && rule.value === item.source))
    await createSecurityAccessRule({
      listType: "blacklist",
      matchType: "ip",
      matchValue: item.source,
      scope: "all_sip",
      status: "enabled",
      note: item.reason
    });
...
async function savePolicyPreview() {
  ...
  if (result.code !== 0) {
    Message.error(result.message || "防护策略保存失败");
    return;
  }
  await refreshPreview(false, true);
...
<template>
  <main
    :class="[
      'snow-page',
      'security-preview',
      { 'security-preview--workspace': ['events', 'bans', 'blacklist', 'allowlist'].includes(activeTab) }
    ]"
  >
    <div class="snow-inner uvp-page-shell-flat security-shell">
      <div class="security-nav">
        <a-tabs v-model:active-key="activeTab" class="security-tabs" type="line">
          <a-tab-pane key="overview"
            ><template #title><ShieldCheck :size="14" />安全总览</template></a-tab-pane
          >
          <a-tab-pane key="policy"
            ><template #title><SlidersHorizontal :size="14" />防护策略</template></a-tab-pane
          >
          <a-tab-pane key="events"
            ><template #title><TriangleAlert :size="14" />风险事件</template></a-tab-pane
          >
          <a-tab-pane key="bans"
            ><template #title><Ban :size="14" />自动封禁</template></a-tab-pane
          >
          <a-tab-pane key="blacklist"
            ><template #title><ShieldOff :size="14" />黑名单</template></a-tab-pane
          >
          <a-tab-pane key="allowlist"
            ><template #title><UserRoundCheck :size="14" />白名单</template></a-tab-pane
          >
        </a-tabs>
        <div class="tab-actions">
          <span :class="['security-live-status', `security-live-status--${liveStatus.color}`]"
            ><span class="live-dot" />{{ liveStatus.label }}</span
          >
          <a-tooltip content="刷新安全数据">
            <a-button class="uvp-page-action-btn uvp-refresh-btn" aria-label="刷新安全数据" @click="refreshPreview(true)"
              ><template #icon><RefreshCw :size="16" /></template>{{ live ? `刷新 ${refreshCountdown}s` : "刷新" }}</a-button
            >
          </a-tooltip>
          <a-button v-if="activeTab === 'policy'" type="primary" @click="savePolicyPreview"
            ><template #icon><Check :size="16" /></template>保存策略</a-button
          >
          <a-button v-else-if="activeTab === 'bans'" type="primary" @click="activeTab = 'policy'"
            ><template #icon><SlidersHorizontal :size="16" /></template>调整策略</a-button
          >
          <a-button
            v-else
            class="uvp-page-action-btn uvp-create-btn"
            type="primary"
            @click="openRuleDrawer(activeTab === 'allowlist' ? 'allowlist' : 'blacklist')"
            ><template #icon><Plus :size="16" /></template>{{ activeTab === "allowlist" ? "添加白名单" : "添加黑名单" }}</a-button
          >
        </div>
      </div>
      ...
      <template v-if="activeTab === 'overview'">
        <section class="posture-panel uvp-system-panel" aria-label="当前防护状态">
          <div class="posture-main">
            ...
            <div>
              <span class="section-label">当前安全态势</span>
              <h2>{{ selectedModeConfig.title }}模式已启用</h2>
              <p>{{ selectedModeConfig.summary }}主机防火墙状态以 Agent 实际回报为准，云厂商边界防护尚未接入。</p>
            </div>
          </div>
          <button class="advice-block" type="button" @click="activeTab = 'policy'">
            <span><TriangleAlert :size="18" /></span>
            <span
              ><strong>{{ attentionCount ? `还有 ${attentionCount} 项待处理` : "当前没有待处理项" }}</strong
              ><small>{{
                attentionCount ? "查看自动封禁和访问名单，确认是否需要长期处理。" : "自动封禁和访问名单当前没有需要复核的项目。"
              }}</small></span
            >
        ...
        <section class="metric-grid" aria-label="安全指标">
          <article>
            <span>已识别安全事件</span><strong>{{ recognizedCount }}</strong
            ><small class="warning"><Activity :size="13" />来自安全事件接口聚合</small>
          </article>
          <article>
            <span>应用层已拦截</span><strong>{{ blockedCount }}</strong
            ><small class="success"><ShieldCheck :size="13" />拒绝与封禁动作累计</small>
          </article>
          <article>
            <span>生效中自动封禁</span><strong>{{ banTotal }}</strong
            ><small
              ><Ban :size="13" />主机防火墙已生效 {{ overviewBans.filter(item => item.firewallState === "已生效").length }}</small
            >
          </article>
          <article>
            <span>主机防火墙</span><strong class="status-value">{{ securityAgent.connected ? "在线" : "降级" }}</strong
            ><small :class="securityAgent.connected ? 'success' : 'warning'"
              ><BrickWall :size="13" />{{ securityAgent.appliedRules }} 条动态规则</small
            >
          </article>
        </section>
        ...
        <section class="overview-grid">
          <article class="uvp-system-panel chart-panel">
            <div class="panel-heading">
              <div>
                <span class="section-label">攻击趋势</span>
                <h3>识别与封禁</h3>
              </div>
              <div class="period-switch" aria-label="趋势周期">
                <button
                  v-for="item in trendPeriods"
                  :key="item"
                  type="button"
            ...
            <div class="chart-legend">
              <span><i class="detected" />识别</span><span><i class="blocked" />封禁</span><b>安全事件接口</b>
            </div>
            <div v-if="trendData.hasData" ref="chartElement" class="trend-chart" aria-label="攻击识别与封禁趋势图" />
            <div v-else class="empty-state chart-empty" role="status">
              <span class="empty-state-icon"><Activity :size="22" /></span><strong>暂无趋势数据</strong
              ><small>当前时间范围内还没有安全事件，接口收到数据后会自动更新。</small>
            </div>
          </article>
          ...
          <article class="uvp-system-panel defense-panel">
            <div class="panel-heading">
              <div>
                <span class="section-label">三层防护</span>
                <h3>防护链路</h3>
              </div>
              <a-tag color="blue">{{ enabledDefenseLayers }} / 3 已启用</a-tag>
            </div>
            <div class="defense-chain">
              <div class="defense-item pending">
                <span><CloudCog :size="19" /></span>
                <div><strong>云边界防护</strong><small>待接入云厂商 API</small></div>
                <a-tag color="orange">规划中</a-tag>
              </div>
              <i class="chain-line" />
              <div class="defense-item" :class="{ active: securityAgent.connected }">
                <span><BrickWall :size="19" /></span>
                <div><strong>主机防火墙</strong><small>uvp-firewall-agent · nftables</small></div>
                <a-tag :color="securityAgent.connected ? 'green' : 'orange'"
                  ><Check :size="12" />{{ securityAgent.connected ? "在线" : "降级" }}</a-tag
                >
              </div>
              <i class="chain-line active" />
              <div class="defense-item active">
                <span><Fingerprint :size="19" /></span>
                <div><strong>应用协议防护</strong><small>限速 · 鉴权 · 风险评分</small></div>
                <a-tag color="green"><Check :size="12" />保护</a-tag>
              </div>
            </div>
          </article>
        </section>
        ...
        <section class="overview-grid lower-grid">
          <article class="uvp-system-panel signal-panel">
            <div class="panel-heading">
              <div>
                <span class="section-label">实时信号</span>
                <h3>最近高频来源</h3>
              </div>
              <button class="live-toggle" type="button" :aria-pressed="live" @click="live = !live">
                <Pause v-if="live" :size="14" /><Play v-else :size="14" />{{ live ? "自动刷新" : "已暂停" }}
              </button>
            </div>
            <div v-if="!overviewEvents.length" class="empty-state signal-empty" role="status">
              <span class="empty-state-icon"><Activity :size="22" /></span><strong>暂无高频来源</strong
              ><small>安全事件接口暂未发现重复风险来源，收到数据后会在这里显示。</small>
            </div>
            <button
              v-for="event in overviewEvents.slice(0, 3)"
              v-else
              :key="event.id"
              class="signal-row"
              type="button"
            ...
            >
              ...
              <span
                ...
                ><small>{{ event.method }} · {{ event.rule }}</small></span
              >
          ...
          <article class="uvp-system-panel attention-panel">
            <div class="panel-heading">
              <div>
                <span class="section-label">待处理</span>
                <h3>安全建议</h3>
              </div>
              <span class="attention-count">{{ attentionCount }}</span>
            </div>
            <button v-if="banTotal" class="attention-row" type="button" @click="activeTab = 'bans'">
              <span class="attention-icon danger"><Ban :size="18" /></span
              ><span
                ><strong>{{ banTotal }} 条自动封禁待复核</strong
                ><small>新自动封禁永久生效，请核对来源后决定是否人工解封。</small></span
              ><ChevronRight :size="16" />
            </button>
            <button v-if="allowRuleTotal" class="attention-row" type="button" @click="activeTab = 'allowlist'">
              <span class="attention-icon warning"><UserRoundCheck :size="18" /></span
              ><span
                ><strong>{{ allowRuleTotal }} 条白名单规则</strong><small>可信出口加入白名单后可减少误判。</small></span
              ><ChevronRight :size="16" />
            </button>
            <div v-if="!attentionCount" class="empty-state attention-empty" role="status">
              <span class="empty-state-icon"><CheckCircle2 :size="22" /></span><strong>当前没有待处理项</strong
              ><small>安全事件、自动封禁和访问名单会在接口刷新后更新。</small>
            </div>
          </article>
        </section>
      </template>
      ...
      <section v-else-if="activeTab === 'events'" class="workspace-panel">
        <s-layout-search class="security-list-search event-search-panel">
          <template #fields>
            <div class="security-list-filter security-list-filter--keyword">
              <a-input
                v-model="eventSearch"
                allow-clear
                placeholder="筛选本页 IP、方法或 User-Agent"
                aria-label="筛选本页风险事件"
                ><template #prefix><Search :size="15" /></template
              ></a-input>
            </div>
            <div class="security-list-filter security-list-filter--status">
              <a-select v-model="eventSeverity" aria-label="风险等级"
                ><a-option>全部风险</a-option><a-option>高危</a-option><a-option>中危</a-option
                ><a-option>低危</a-option></a-select
              >
            </div>
          </template>
        ...
        <a-table
          class="security-table uvp-data-table"
          :data="filteredEvents"
          row-key="id"
          :pagination="eventPagination"
          :scroll="{ x: 1050, y: '100%' }"
          @page-change="handleEventPageChange"
          @page-size-change="handleEventPageSizeChange"
        >
          <template #columns>
            <a-table-column title="风险" :width="90"
              ><template #cell="{ record }"
                ><span :class="['severity', record.severity === '高危' ? 'high' : record.severity === '中危' ? 'medium' : 'low']"
                  ><i />{{ record.severity }}</span
                ></template
              ></a-table-column
            >
            <a-table-column title="来源" :width="170"
              ><template #cell="{ record }"
                ><strong class="mono">{{ record.source }}</strong
                ><small class="cell-subline">{{ record.location }}</small></template
              ></a-table-column
            >
      ...
      <section v-else-if="activeTab === 'bans'" class="workspace-panel">
        <s-layout-search class="security-list-search ban-search-panel">
          <template #fields>
            <div class="security-list-filter security-list-filter--keyword">
              <a-input v-model="banSearch" allow-clear placeholder="筛选本页来源、原因或策略" aria-label="筛选本页自动封禁"
                ><template #prefix><Search :size="15" /></template
              ></a-input>
            </div>
            <div class="security-list-filter security-list-filter--status">
              <a-select v-model="banFirewallState" aria-label="主机防火墙状态"
                ><a-option>全部状态</a-option><a-option>已生效</a-option><a-option>待同步</a-option
                ><a-option>同步失败</a-option></a-select
              >
            </div>
          </template>
        ...
        <a-table
          class="security-table uvp-data-table ban-table"
          :data="filteredAutoBans"
          row-key="id"
          :pagination="banPagination"
          :scroll="{ x: 1060, y: '100%' }"
          @page-change="handleBanPageChange"
          @page-size-change="handleBanPageSizeChange"
        >
          <template #columns>
            <a-table-column title="来源" :width="160"
              ...
              ></a-table-column
            >
            <a-table-column title="进入原因" :width="280"
              ><template #cell="{ record }"
                ><strong class="ban-reason">{{ record.reason }}</strong
                ><small class="cell-subline">{{ record.evidence }}</small></template
              ></a-table-column
            >
            <a-table-column title="触发策略" :width="110"
              ><template #cell="{ record }"
                ><a-tag :color="record.mode === '严格' ? 'red' : 'blue'">{{ record.mode }}模式</a-tag
                ><small class="cell-subline">{{ record.method }}</small></template
              ></a-table-column
            >
            <a-table-column title="防护位置" :width="170"
              ><template #cell="{ record }"
                ><span class="enforcement-tags"
                  ><a-tag color="green">应用层已拦截</a-tag
                  ><a-tag
                    :color="record.firewallState === '已生效' ? 'green' : record.firewallState === '待同步' ? 'orange' : 'red'"
                    >主机防火墙{{ record.firewallState }}</a-tag
                  ></span
                ></template
              ></a-table-column
            >
            <a-table-column title="封禁时间" :width="110"
              ><template #cell="{ record }"
                ><span>{{ record.createdAt }}</span
                ><small class="cell-subline">{{ record.expires }}</small></template
              ></a-table-column
            >
            <a-table-column title="封禁后拦截" :width="100"
              ><template #cell="{ record }"
                ><strong>{{ record.blocked }} 次</strong></template
              ></a-table-column
            >
            <a-table-column title="操作" :width="130"
              ><template #cell="{ record }"
                ><span class="ban-actions"
                  ><a-tooltip content="查看关联风险事件"
                    ><a-button size="small" aria-label="查看关联风险事件" @click="showBanEvents(record)"
                      ><template #icon><Search :size="14" /></template></a-button></a-tooltip
                  ><a-tooltip content="转为手动黑名单"
                    ><a-button size="small" aria-label="转为手动黑名单" @click="promoteToBlacklist(record)"
                      ><template #icon><Ban :size="14" /></template></a-button></a-tooltip
                  ><a-tooltip content="解除自动封禁"
                    ><a-button size="small" status="danger" aria-label="解除自动封禁" @click="unbanPreview(record)"
                      ><template #icon><ShieldOff :size="14" /></template></a-button></a-tooltip></span></template
            ></a-table-column>
          </template>
        </a-table>
      </section>
      ...
      <section v-else-if="activeTab === 'blacklist' || activeTab === 'allowlist'" class="workspace-panel">
          ...
          <template #fields>
            <div class="security-list-filter security-list-filter--keyword">
              <a-input
                v-model="ruleSearch"
                allow-clear
                placeholder="筛选本页匹配内容或备注"
                :aria-label="`筛选本页${activeTab === 'blacklist' ? '黑名单' : '白名单'}`"
                ><template #prefix><Search :size="15" /></template
              ></a-input>
            </div>
            <div class="security-list-filter">
              <a-select v-model="ruleTypeFilter" aria-label="规则类型"
                ><a-option>全部类型</a-option><a-option>IP</a-option><a-option>CIDR</a-option
                ><a-option>User-Agent</a-option></a-select
              >
            </div>
            <div class="security-list-filter security-list-filter--status">
              <a-select v-model="ruleStatusFilter" aria-label="规则状态"
                ><a-option>全部状态</a-option><a-option>已启用</a-option><a-option>已停用</a-option></a-select
              >
            </div>
          </template>
        ...
        >
          <template #columns>
            ...
            <a-table-column title="匹配内容" :width="210"
              ><template #cell="{ record }"
                ><span class="mono">{{ record.value }}</span></template
              ></a-table-column
            >
            ...
            <a-table-column title="状态" :width="100"
              ><template #cell="{ record }"
                ><a-switch :model-value="record.enabled" size="small" @change="toggleRuleFromEvent(record, $event)" /></template
            ></a-table-column>
            <a-table-column title="操作" :width="80" fixed="right"
              ><template #cell="{ record }"
                ><a-tooltip content="删除规则"
                  ><a-button type="text" status="danger" aria-label="删除规则" @click="removeRule(activeTab, record.id)"
                    ><template #icon><Trash2 :size="16" /></template></a-button></a-tooltip></template
            ></a-table-column>
          </template>
        </a-table>
      </section>
      ...
      <section v-else class="workspace-panel policy-workspace">
        <div class="mode-selector" role="radiogroup" aria-label="防护模式">
          <button
            v-for="mode in protectionModes"
            :key="mode.key"
            type="button"
            :class="['mode-option', `mode-${mode.key}`, { active: selectedMode === mode.key }]"
          ...
          >
            ...
            <span class="mode-copy"
              ><span class="mode-heading"
                ><strong>{{ mode.title }}</strong
                ><em>{{ mode.badge }}</em></span
        ...
        <section :class="['mode-explainer', `mode-${selectedModeConfig.key}`]" aria-live="polite">
          <div class="mode-explainer-heading">
            ...
            <div>
              <small>{{ selectedModeConfig.title }}模式启用后</small>
              <h3>{{ selectedModeConfig.headline }}</h3>
              <p>{{ selectedModeConfig.summary }}</p>
            </div>
          </div>
          <div class="mode-result-grid">
            <article>
              <span>入口与速率</span><strong>{{ selectedModeConfig.entryResult }}</strong>
            </article>
            <article>
              <span>协议鉴权</span><strong>{{ selectedModeConfig.protocolResult }}</strong>
            </article>
            <article>
              <span>风险升级</span><strong>{{ selectedModeConfig.banResult }}</strong>
            </article>
          </div>
        </section>
        <div class="policy-grid">
          <div class="policy-section">
            <div class="policy-title">
              ...
              <div>
                <h3>自动防护规则</h3>
                <p>控制异常来源何时被识别并升级处理。</p>
              </div>
            </div>
            <div class="setting-row">
              <span
                ><strong>重复攻击自动封禁</strong
                ><small
                  >{{
                    selectedMode === "observe" ? "观察模式仅记录" : "保护/严格模式先拒绝；自动永久 IP 封禁仍需来源验证门禁"
                  }}；短期 {{ shortWindowInviteRuleLabel }}；固定低频 {{ lowFrequencyInviteRuleLabel }}。</small
                ></span
              ><a-tag :color="selectedMode === 'observe' ? 'orange' : 'green'">{{
                selectedMode === "observe" ? "观察模式不执行" : automaticBanTTLLabel
              }}</a-tag>
            </div>
            <div class="setting-row">
              <span
                ><strong>注册失败处理</strong
                ><small
                  >所有模式都校验 REGISTER 格式、鉴权和设备 ID；密码错误或单一非法 ID
                  会拒绝并提示检查配置，修正后可重新注册。</small
                ></span
              ><a-tag color="blue">可修正重试</a-tag>
            </div>
            <div class="setting-row">
              <span
                ><strong>扫描器特征识别</strong
                ><small>User-Agent 仅作为辅助信号，不作为可信身份；非法 ID 枚举单独计为高危，不直接等同于永久封 IP。</small></span
              ><a-tag color="green">协议事件接口</a-tag>
            </div>
            <div class="setting-row">
              <span
            ...
            <div class="setting-row">
              <span
                ><strong>来源验证门禁</strong><small>{{ sourceVerificationNote }}</small></span
              ><a-tag color="blue">自动校验</a-tag>
            </div>
          </div>
          <div class="policy-section">
            <div class="policy-title">
              <span><BrickWall :size="18" /></span>
              <div>
                <h3>主机防火墙</h3>
                <p>阻止已确认的攻击流量继续进入服务进程。</p>
              </div>
            </div>
            <div class="setting-row">
              <span
                ><strong>联动主机防火墙</strong
                ><small
                  >应用层确认攻击后，只有来源通过验证门禁且不是成功认证共享出口，才在操作系统网络入口永久封禁来源 IP</small
                ></span
              ><a-tag :color="securityAgent.connected ? 'green' : 'orange'">{{
                securityAgent.connected ? "Agent 已连接" : "Agent 未连接"
              }}</a-tag>
            </div>
            <div class="agent-status">
              <span :class="['agent-icon', { warning: !securityAgent.connected }]"
                ><CheckCircle2 v-if="securityAgent.connected" :size="22" /><TriangleAlert v-else :size="22" /></span
              ><span
                ><strong>{{ agentStatus.title }}</strong
                ><small>{{ agentStatus.detail }}</small></span
              ><a-tag :color="agentStatus.color">{{ agentStatus.label }}</a-tag>
            </div>
            <div class="ttl-row">
              <span
                ><strong>自动封禁有效期</strong><small>新自动封禁永久生效，需人工解封；历史限时记录保留原到期时间。</small></span
              ><a-tag color="orange">{{ automaticBanTTLLabel }}</a-tag>
            </div>
            <div class="cloud-roadmap">
              <CloudCog :size="18" /><span
                ><strong>云厂商防火墙联动</strong><small>当前系统未接入云厂商 API，暂不宣称已生效。</small></span
    ...
    <a-modal
      ...
      modal-class="uvp-system-dialog security-rule-dialog"
      :title="editingRuleKind === 'blacklist' ? '新增黑名单规则' : '新增白名单规则'"
      :width="560"
      :footer="false"
      unmount-on-close
    >
      <div class="drawer-intro">
        <span :class="editingRuleKind"
          ><Ban v-if="editingRuleKind === 'blacklist'" :size="20" /><UserRoundCheck v-else :size="20"
        /></span>
        <div>
          <strong>{{ editingRuleKind === "blacklist" ? "拒绝风险来源" : "放行可信来源" }}</strong>
          <p>规则会写入安全策略并在应用层即时生效；User-Agent 规则不会写入主机防火墙。</p>
        </div>
      </div>
      <a-form :model="ruleFormModel" layout="vertical" class="rule-form">
        <a-form-item label="规则类型"
          ><a-radio-group v-model="ruleType" type="button"
            ><a-radio value="IP">IP</a-radio><a-radio value="CIDR">CIDR</a-radio
            ><a-radio value="User-Agent">User-Agent</a-radio></a-radio-group
          ></a-form-item
        >
        ...
        <a-form-item label="有效期"
          ><a-select v-model="ruleExpiry"
            ><a-option>10 分钟</a-option><a-option>1 小时</a-option><a-option>1 天</a-option><a-option>7 天</a-option
            ><a-option>永久</a-option></a-select
          ></a-form-item
        >
      </a-form>
      <a-alert v-if="editingRuleKind === 'allowlist'" type="warning"
        >白名单只降低来源风险分，不会绕过 SIP 格式、鉴权和设备身份校验。</a-alert
      >
      <div class="drawer-actions">
        <a-button @click="ruleDrawerVisible = false">取消</a-button><a-button type="primary" @click="saveRule">保存规则</a-button>
      </div>
    </a-modal>
  </main>
</template>
...
.security-live-status--green {
  color: var(--uvp-success);
}
.security-live-status--orange {
  color: var(--uvp-warning);
}
.security-live-status--red {
...
.shield-orbit::after {
  ...
  border-radius: 50%;
  animation: shield-pulse 2.2s ease-out infinite;
}
...
@media (width <= 1180px) {
  ...
  .advice-block {
    border-top: 1px solid var(--uvp-warning-border);
    border-left: 0;
  }
  .metric-grid {
    grid-template-columns: repeat(2, minmax(0, 1fr));
...
@media (width <= 520px) {
  ...
  .tab-actions :deep(.arco-tag) {
    display: none;
  }
  .tab-actions :deep(.arco-btn) {
    flex: 1;
  }
  :deep(.security-tabs .arco-tabs-nav-tab-list) {
    gap: 0;
