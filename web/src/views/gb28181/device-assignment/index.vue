...
const summary = ref({
  ...
  assignedCount: 0,
  unassignedCount: 0,
  departments: [] as Array<{ deptId: number; directCount: number; subtreeCount: number }>
});
...
const openBatchShare = () => {
  ...
  shareDevices.value = selection.selectedDevices.value.map(d => ({ id: d.id, deviceId: d.deviceId, name: d.name }));
  shareVisible.value = true;
};
      ...
      <s-fold-page :width="280">
        <template #sider>
          <div class="left-box uvp-tree-panel">
            ...
            <a-input class="uvp-tree-panel__search" v-model="deptSearchKeyword" placeholder="请输入部门名称" allow-clear>
              <template #prefix>
                <icon-search />
              </template>
            </a-input>
              ...
              >
                ...
                <template #title="node">
                  <div class="dept-tree-node">
                    ...
                    <a-link v-if="canAssign" class="dept-tree-node__assign" @click.stop="openDeptAssign(node)">
                      分配本部门设备
                    </a-link>
                  </div>
                </template>
              ...
              <div v-if="guideExpanded" class="workbench-guide__body">
                <div class="workbench-guide__meaning">
                  <div class="workbench-guide__meaning-item">
                    <span class="workbench-guide__meaning-icon workbench-guide__meaning-icon--assign"
                      ><ListChecks :size="15"
                    /></span>
                    <div><strong>调整归属</strong><span>改变设备的管理部门，影响后续谁负责维护和管理。</span></div>
                  </div>
                  <div class="workbench-guide__meaning-item">
                    <span class="workbench-guide__meaning-icon workbench-guide__meaning-icon--share"
                      ><ShieldCheck :size="15"
                    /></span>
            ...
            <s-layout-search class="account-search-panel">
              <template #fields>
                <div class="segmented workbench-views" role="tablist" aria-label="权限视图">
                  <button type="button" :class="{ active: mainView === 'assignment' }" @click="mainView = 'assignment'">
                    设备归属
                  </button>
                  <button type="button" :class="{ active: mainView === 'sharing' }" @click="mainView = 'sharing'">
                    共享授权
                  </button>
                </div>
                ...
                <a-table-column title="操作" :width="190" align="center" fixed="right" cell-class="operation-column">
                  <template #cell="{ record }">
                    <div class="device-operation-actions">
                      <a-link v-if="canAssign" class="uvp-table-action uvp-table-action--assign" @click="openRowAssign(record)">
                        <template #icon><UserRoundCog :size="13" /></template>
                        调整归属
                      </a-link>
                      <a-link v-if="canShare" class="uvp-table-action uvp-table-action--permission" @click="openRowShare(record)">
