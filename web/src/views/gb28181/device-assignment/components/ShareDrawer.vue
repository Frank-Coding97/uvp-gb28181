...
const currentOptions = computed(() => {
  ...
  return [...byKey.values()].filter(
    item => !keyword || item.name.toLowerCase().includes(keyword) || item.deptName?.toLowerCase().includes(keyword)
  );
});
  ...
  <a-drawer v-model:visible="visible" :width="560" :title="title" unmount-on-close>
    <div class="share-drawer">
      ...
      <a-radio-group
        ...
        type="button"
        @change="(value: string | number) => switchTargetType(value as GrantTargetType)"
      >
      ...
      <div class="share-drawer__search">
        <a-input
          ...
          allow-clear
          :placeholder="mode === 'add' ? '搜索可授权目标' : '从已有授权中筛选'"
      ...
      <a-pagination
        ...
        "
      />
      <div v-if="draftCount" class="share-drawer__draft">
        <Check :size="15" /> 已暂存 {{ draftCount }} 个目标，点击保存后才会生效
      </div>
    </div>
    <template #footer>
      <a-space>
...
.share-drawer__devices strong,
.share-drawer__devices span {
  display: block;
}
.share-drawer__devices strong {
...
.share-drawer__target:hover,
.share-drawer__target.checked {
  border-color: color-mix(in srgb, var(--uvp-brand) 35%, var(--uvp-panel-border));
  background: var(--uvp-brand-soft);
}
.share-drawer__target.disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
.share-drawer__target-check {
  display: grid;
  flex: 0 0 20px;
  width: 20px;
  height: 20px;
...
.share-drawer__target.checked .share-drawer__target-check {
  border-color: var(--uvp-brand);
  background: var(--uvp-brand);
}
.share-drawer__target-main {
  min-width: 0;
  flex: 1;
}
.share-drawer__target-main strong,
.share-drawer__target-main small {
  display: block;
  overflow: hidden;
