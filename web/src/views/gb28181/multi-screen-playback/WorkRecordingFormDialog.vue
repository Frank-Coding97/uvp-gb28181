...
import { getWorkRecordingBatchForm, getWorkRecordingForm, saveWorkRecordingBatchForm, saveWorkRecordingForm, type WorkRecordingForm, type WorkRecordingFormDetail, type WorkRecordingBatchFormDetail } from "@/api/gb28181-work-recording";
const props = defineProps<{ visible: boolean; jobId?: string; batchId?: string; channelName: string; canEdit: boolean }>();
...
const detail = ref<(WorkRecordingFormDetail | WorkRecordingBatchFormDetail) | null>(null);
...
function accept(data: WorkRecordingFormDetail | WorkRecordingBatchFormDetail) {
    detail.value = data;
    fields.value = { ...data.form, workPersonnel: (data.form.workPersonnel || []).join("、") };
    baseline.value = JSON.stringify(fields.value);
}
function responseTarget(data: WorkRecordingFormDetail | WorkRecordingBatchFormDetail) {
    return "batchId" in data ? data.batchId : data.jobId;
}
async function load() {
    const current = ++sequence;
    const targetId = props.batchId || props.jobId || "";
    detail.value = null;
    loading.value = true;
    error.value = "";
    notice.value = "";
    try {
        const response = props.batchId ? await getWorkRecordingBatchForm(targetId) : await getWorkRecordingForm(targetId);
        if (response.code !== 0) throw new Error(response.message || "表单读取失败");
        if (current !== sequence) return;
        if (responseTarget(response.data) !== targetId) throw new Error("返回的作业与当前表单不一致");
...
async function save() {
    ...
    notice.value = "";
    try {
        const response = props.batchId ? await saveWorkRecordingBatchForm(targetId, detail.value.formVersion, form) : await saveWorkRecordingForm(targetId, detail.value.formVersion, form);
...
watch(() => [props.visible, props.jobId, props.batchId] as const, ([visible, jobId, batchId]) => {
    if (visible && (jobId || batchId)) void load();
...
<template>
    <a-modal :visible="visible" :width="820" :footer="false" :mask-closable="false" @cancel="close">
        <template #title>{{ props.batchId ? '作业台账' : '放线作业记录' }}</template>
