<template>
  <div v-if="!canView" class="recording-plans-denied" role="alert">当前账号没有录像计划查看权限。</div>
  <div v-else class="recording-schedules-page">
    <div class="recording-schedules-shell">
      <div class="recording-schedules-toolbar">
        <div class="segmented schedule-view-switch" role="tablist" aria-label="录像计划视图">
          <button type="button" :class="{ active: activeView === 'plans' }" :aria-pressed="activeView === 'plans'" @click="selectInternalView('plans')">
			<CalendarClock :size="14" />计划管理<span>{{ planTotal }}</span>
          </button>
          <button type="button" :class="{ active: activeView === 'status' }" :aria-pressed="activeView === 'status'" @click="selectInternalView('status')">
		  ...
		  <a-table class="uvp-data-table schedule-data-table" row-key="id" :data="plans" :loading="plansLoading" :bordered="false" :pagination="planPagination" :scroll="planTableScroll" @page-change="handlePlanPageChange" @page-size-change="handlePlanPageSizeChange">
            <template #columns>
              ...
              <a-table-column title="已应用通道" :width="126" align="center">
                <template #cell="{ record }">
                  <a-link v-if="canAssign" class="channel-count-link" :title="`查看并管理 ${record.channelCount} 个已应用通道`" @click="openAssign(record)">{{ record.channelCount }} 个</a-link>
                  <span v-else>{{ record.channelCount }} 个</span>
                </template>
              </a-table-column>
              <a-table-column title="状态" :width="110" align="center">
                <template #cell="{ record }">
                  <a-switch v-if="canMaintain" v-model="record.enabled" :aria-label="`${record.name}${record.enabled ? '停用' : '启用'}`" @change="notifyPlanStatusChange(record)" />
                  <a-badge v-else :status="record.enabled ? 'success' : 'normal'" :text="record.enabled ? '已启用' : '已停用'" />
                </template>
              </a-table-column>
            ...
            <div class="execution-channel-table-wrap">
			  <a-table class="uvp-data-table schedule-data-table execution-data-table" row-key="id" :data="filteredExecutionChannels" :loading="executionChannelsLoading" :bordered="false" :pagination="executionPagination" :scroll="executionTableScroll" @page-change="handleExecutionPageChange" @page-size-change="handleExecutionPageSizeChange">
                <template #columns>
                  ...
                  <a-table-column title="证据" :width="86" align="center" fixed="right"><template #cell="{ record }"><a-link @click="openChannelEvidence(record)">诊断</a-link></template></a-table-column>
                </template>
  ...
  <a-modal v-model:visible="evidenceVisible" :title="evidenceTitle" :footer="false" width="720px">
    <a-spin :loading="evidenceLoading" class="recording-evidence-spin">
      <pre class="recording-evidence">{{ evidenceText }}</pre>
    </a-spin>
  </a-modal>
</template>
...
import {
  createRecordingPlan, deleteRecordingPlan, diagnoseRecordingPlanChannel, getRecordingPlan, getRecordingPlanChannelTimeline, listRecordingPlanExecutionChannels,
...
function safeContext(value: unknown, maxLength: number) {
  ...
  const normalized = String(scalar).trim().slice(0, maxLength);
  return /[:][/][/]|[\u0000-\u001f\u007f]/.test(normalized) ? "" : normalized;
}

const recordingContextKeyword = computed(() => safeContext(props.stream, 128));
const recordingContextNodeId = computed(() => {
  const value = safeContext(props.nodeId, 20);
...
const canView = computed(() => hasPermission("gb28181:recording-plan:view"));
const canMaintain = computed(() => hasPermission("gb28181:recording-plan:maintain"));
const canAssign = computed(() => hasPermission("gb28181:recording-plan:assign"));
...
const activeView = ref<"plans" | "status">(recordingContextKeyword.value ? "status" : "plans");
...
const statusFilters = reactive({ keyword: recordingContextKeyword.value, online: "", state: "" });
...
const evidenceTitle = ref("通道诊断与时间线");
const evidenceText = ref("尚未加载证据");
let generation = 0;
...
function selectInternalView(view: "plans" | "status") {
  if (activeView.value === view) return;
  activeView.value = view;
  if (!props.active || !canView.value) return;
  if (view === "plans") void loadPlans();
...
async function confirmAssignment(payload: { planId: string; scope: "device" | "channel"; targetIds: number[]; channelCount: number }) {
  ...
  Message.success(payload.scope === "device"
	...
	await Promise.all([loadPlans(), loadExecutionPlans(false)]);
}
...
async function loadExecutionPlans(append: boolean) {
	...
	try {
		...
		if (requestGeneration === generation) executionPlansLoading.value = false;
	}
}
...
async function loadDetailChannels() {
	if (!activePlan.value || !props.active || !canView.value) return;
	const requestGeneration = generation;
	...
	try {
		...
		if (requestGeneration === generation) detailChannelsLoading.value = false;
	}
}

async function openChannelEvidence(channel: ScheduleChannel) {
	if (!props.active || !canView.value) return;
	evidenceVisible.value = true;
	evidenceLoading.value = true;
	evidenceTitle.value = `${channel.name} · 诊断与时间线`;
	try {
		const [diagnosis, timeline] = await Promise.all([
			diagnoseRecordingPlanChannel(Number(channel.id)),
			getRecordingPlanChannelTimeline(Number(channel.id), { page: 1, pageSize: 50 })
		]);
		evidenceText.value = JSON.stringify({ diagnosis: diagnosis.data, timeline: timeline.data }, null, 2);
	} finally {
...
watch(recordingContextKeyword, value => {
	if (!value || statusFilters.keyword === value) return;
	statusFilters.keyword = value;
	activeView.value = "status";
	if (props.active && canView.value) void loadExecutionPlans(false);
});
...
.recording-plans-denied { display: grid; height: 100%; min-height: 260px; place-items: center; color: var(--zlm-danger-600); background: var(--uvp-panel-bg); border: 1px solid var(--uvp-panel-border); border-radius: var(--uvp-panel-radius); }
.recording-evidence-spin { display: block; min-height: 260px; }
.recording-evidence { max-height: 520px; margin: 0; overflow: auto; color: var(--zlm-text-2); font-family: var(--zlm-font-mono); font-size: 12px; line-height: 1.6; white-space: pre-wrap; overflow-wrap: anywhere; }
