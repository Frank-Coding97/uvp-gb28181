...
import { runtimeSnapshot } from "./workbench/chart/runtimeChart";
...
describe("runtime overview state", () => {
  it("derives viewers, throughput, FD and recording counts with the sample timestamp", () => {
    expect(runtimeSnapshot(runtime(2))).toMatchObject({
      streamCount: 1,
      viewerCount: 2,
      throughput: 1024,
      sessionCount: 7,
  ...
  it("uses typed polling and keeps a single-node operational overview", () => {
    ...
    expect(source).toContain("<MediaRateArea");
    expect(source).toContain(':samples="mediaRateSamples"');
    expect(source).toContain("runtimeMediaRateSamples(runtime.value)");
    expect(source).not.toContain("runtimeMediaRateSamples(history.value.samples)");
    ...
    expect(source).toContain("trendSamples");
    expect(source).not.toContain("appendRuntimeSnapshot");
    expect(source).not.toContain("appendObjectStatisticSample");
    expect(source).not.toContain("history.value");
