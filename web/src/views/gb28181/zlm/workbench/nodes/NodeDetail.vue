...
function setView(view: string) {
  ...
  void router.push({ path, query: { ...route.query, view } });
}
...
<template>
  <div class="snow-fill">
    <div class="snow-fill-inner uvp-page-shell-flat node-detail-shell">
      <section class="node-detail-view" aria-label="媒体节点详情">
        ...
        <template v-else>
          ...
          <template v-if="node">
            <nav class="detail-tabs" aria-label="节点详情视图"><button v-for="view in allowedViews" :key="view" type="button" :class="{ 'is-active': currentView === view }" :aria-current="currentView === view ? 'page' : undefined" @click="setView(view)">{{ view === 'overview' ? '概览' : view === 'runtime' ? '运行监控' : '服务配置' }}</button></nav>
