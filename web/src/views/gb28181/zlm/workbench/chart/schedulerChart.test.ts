  ...
  it("aggregates only the returned filtered sample and keeps its filter summary", () => {
    const state = buildSchedulerChartState(
      [
        {
          id: 1,
          happenedAt: "2026-08-30T10:00:00Z",
          algorithm: "weighted",
          nodeID: 2,
          nodeName: "节点 2",
          streamID: "cam-1",
        ...
        {
          id: 2,
          happenedAt: "2026-08-30T10:01:00Z",
          algorithm: "weighted",
          nodeID: 0,
          nodeName: "",
          streamID: "cam-2",
    ...
    expect(state.filterText).toContain("调度状态 已命中");
    expect(state.filterText).toContain("2 条样本");
    expect(state.resultDistribution).toEqual([
      { category: "已命中", count: 1 },
      { category: "未命中", count: 1 }
    ]);
    expect(state.nodeDistribution).toEqual([{ category: "节点 2", nodeId: 2, count: 1 }]);
    expect(createSchedulerResultSpec(state).data?.[0]?.values).toHaveLength(2);
    expect(createSchedulerNodeSpec(state).data?.[0]?.values).toHaveLength(1);
  });
