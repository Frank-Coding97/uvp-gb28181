...
import { Plus, RefreshCw, ShieldAlert, Workflow } from "lucide-vue-next";
...
<template>
  <section class="ingress-panel" aria-label="FFmpeg 源管理">
    <header class="panel-toolbar">
      <div><h2>FFmpeg 源</h2><p>只展示后端登记模板和脱敏地址；源到目标的完整 URL 不回显。</p></div>
      <a-space>
    ...
    <div class="capability-banner" :data-capability="capability" role="status"><Workflow :size="17" /><strong>{{ capability === 'supported' ? '节点支持' : capability === 'unsupported' ? '节点不支持' : '能力未探测' }}</strong><span>{{ capability === 'supported' ? 'FFmpeg 模板和源列表由节点明确报告可用。' : capability === 'unsupported' ? '节点缺少完整 FFmpeg 管理 API，操作保持禁用。' : '不会把空列表或探测失败当作支持。' }}</span></div>
    ...
    <section class="data-panel">
      ...
      <a-table v-else :data="rows" :loading="loading" :pagination="false" row-key="key" class="uvp-data-table" :scroll="{ x: 1060 }">
        <template #columns>
