...
const createDecision = computed(() =>
  ffmpegCreateDecision(capability.value, templates.value, canManage.value && !scopeBlocked.value)
);
...
const deleteAllowed = computed(() => {
  ...
  return Boolean(
    nodeId.value !== null &&
      deleteTarget.value?.nodeId === nodeId.value &&
      snapshot?.present &&
      snapshot.presenceKnown &&
      snapshot.status === "managed"
  );
});
...
const deleteImpacts = computed(
  () =>
    (deletePreview.value?.snapshot.impacts
...
async function confirmDelete(payload: { fingerprint: string }) {
  ...
  if (
    ...
    !deleteAllowed.value ||
    payload.fingerprint !== deletePreview.value?.fingerprint
  )
    return;
  deleting.value = true;
  try {
  ...
  <section class="ingress-panel" aria-label="FFmpeg 源管理">
    <header class="panel-toolbar">
      <div class="toolbar-actions">
        ...
        <a-button
          ...
          type="primary"
          :disabled="!props.active || !createDecision.allowed || scopeBlocked"
          @click="openCreate"
          ><template #icon><Plus :size="15" /></template>创建源</a-button
        >
      </div>
    </header>
    <div v-if="scopeBlocked" class="scope-warning" role="status">
      <strong>全部节点范围</strong><span>FFmpeg 源列表和写操作需要先选择一个具体节点。</span>
    </div>
    <section class="data-panel">
      ...
      <div v-else-if="scopeBlocked" class="state-box" role="status">
        <strong>请选择具体节点</strong><span>全部节点模式不会发起逐节点 FFmpeg 查询。</span>
      </div>
      <template v-else>
          ...
          <template #columns>
            <a-table-column title="源标识" :width="220"
              ><template #cell="{ record }"
                ...
                <div class="subtle">模板：{{ record.templateKey }}</div></template
              ></a-table-column
            >
            <a-table-column title="源地址摘要" :width="260"
              ><template #cell="{ record }"
                ><code>{{ ffmpegURLText(record.srcUrl) }}</code>
                <div class="subtle">指纹：{{ record.srcUrl.fingerprint }}</div></template
              ></a-table-column
            >
            <a-table-column title="目标地址摘要" :width="260"
              ><template #cell="{ record }"
                ><code>{{ ffmpegURLText(record.dstUrl) }}</code>
                <div class="subtle">指纹：{{ record.dstUrl.fingerprint }}</div></template
              ></a-table-column
            >
            <a-table-column title="来源" :width="130"
              ><template #cell="{ record }"
                ><a-tag :color="record.managed ? 'blue' : 'orange'">{{ record.managed ? "管理台" : "业务/未知" }}</a-tag>
                <div class="subtle">{{ record.createdBy ? `用户 #${record.createdBy}` : "无创建人" }}</div></template
              ></a-table-column
            >
    ...
    <a-modal v-else :visible="deleteVisible" :footer="false" :width="540" unmount-on-close @cancel="closeDelete">
      ...
      <div v-else class="preflight-state" :class="{ 'state-box--error': deleteError }">
        <strong>{{ deleteError ? zlmErrorPresentation(deleteError).label : deleteReason }}</strong
