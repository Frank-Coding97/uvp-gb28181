...
<template>
  <div class="monitoring-panel stream-panel">
    ...
    <a-drawer v-model:visible="previewVisible" :width="760" :footer="false" unmount-on-close @cancel="previewGeneration += 1; previewGrant = null">
      ...
      <div v-if="previewError" class="preview-error" role="alert">{{ previewErrorPresentation.label }}</div><div v-if="previewGrant" class="preview-expiry">授权到期：{{ previewGrant.expiresAt }}；地址仅来自后端授权响应，页面不持有节点 API Secret。</div><PlayWindow :url="previewGrant?.url || ''" :has-audio="true" :zlm-webrtc="previewGrant?.protocol === 'webrtcs'" @error="Message.error($event)" />
    </a-drawer>
