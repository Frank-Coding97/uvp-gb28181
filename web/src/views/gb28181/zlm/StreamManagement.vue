...
import { Camera, Copy, Eye, Radio, Server, ShieldAlert, Users } from "lucide-vue-next";
...
import { buildStreamQuery, streamIdentityKey, streamRouteFilter } from "./streamManagementState";

const route = useRoute();
const router = useRouter();
const context = useZLMContextStore();
const userStore = useUserStoreHook();
const initialRouteFilter = streamRouteFilter(route.query as Record<string, unknown>);
...
const filters = reactive({
  schema: initialRouteFilter?.schema ?? "",
  vhost: initialRouteFilter?.vhost ?? "",
  app: initialRouteFilter?.app ?? "",
  stream: initialRouteFilter?.stream ?? "",
  recording: ""
});
...
const selectedTargets = computed(() => {
  ...
  return rows.value
    .filter(stream => selected.has(streamIdentityKey(stream.nodeId, stream.media)))
...
function currentQuery(nodeId: number) {
  return buildStreamQuery({
    nodeId,
    page: page.value,
...
const { refresh } = useZLMRuntimePolling<ZLMStreamPage>({
  ...
  async load(nodeId, signal) {
    loading.value = true;
    const response = await listZLMStreams(currentQuery(nodeId), signal);
...
function rowKey(stream: ZLMStream) {
  return streamIdentityKey(stream.nodeId, stream.media);
}

function ownershipText(stream: ZLMStream) {
...
function openClose(stream: ZLMStream, force: boolean) {
  ...
  context.selectNode(stream.nodeId);
...
function openBatchClose() {
  if (!canClose.value || selectedTargets.value.length === 0) return;
...
<template>
  <div class="snow-fill">
    <div class="snow-fill-inner uvp-page-shell-flat stream-management">
      <header class="stream-header">
        <div><h1>流管理</h1><p>所有列表、详情和危险操作都通过 UVP 后端；媒体身份始终包含 nodeId/schema/vhost/app/stream。</p></div>
      ...
      <ZLMNodeContextBar :nodes="contextNodes" :query-node-id="route.query.nodeId" :loading="nodesLoading || loading" title="流所在节点" @refresh="loadNodes(); refresh()" />

      <div v-if="readOnly" class="stream-banner" role="status">当前账号仅可查看流运行态；预览、普通关闭和强制关闭按钮按独立权限隐藏。</div>
      <div v-if="pageData?.partial" class="stream-banner stream-banner--warning" role="status">部分节点采集失败，当前仍展示已返回的 {{ pageData.list.length }} 条数据，不清空整页。</div>
      <div v-else-if="loadError && pageData" class="stream-banner stream-banner--warning" role="status">本次刷新失败：{{ errorPresentation.label }}。保留上一次分页、筛选和展开态。</div>
      ...
      <s-layout-search class="stream-search">
        <template #fields>
          ...
          <a-select v-model="filters.recording" allow-clear placeholder="录制状态" class="filter-recording">
            <a-option value="mp4">MP4 录制</a-option><a-option value="hls">HLS 录制</a-option>
          </a-select>
        </template>
        <template #actions>
          <a-button type="primary" @click="applyFilters">查询</a-button><a-button @click="clearFilters">重置</a-button>
          <a-button class="uvp-refresh-btn" :loading="loading" aria-label="刷新媒体流" @click="refresh"><template #icon><icon-refresh /></template>刷新</a-button>
        </template>
        <template #extra>
          <span class="selection-meta">已选 {{ selectedTargets.length }} 路</span>
          <a-button v-if="canClose" status="danger" :disabled="selectedTargets.length === 0" @click="openBatchClose">批量普通关闭</a-button>
        </template>
      </s-layout-search>

      <div v-if="nodesLoading && !nodes.length" class="stream-state" role="status"><a-spin />正在加载媒体节点…</div>
      <div v-else-if="nodesError && !nodes.length" class="stream-state stream-state--error" role="alert"><Server :size="36" /><strong>{{ errorPresentation.label }}</strong><a-button @click="loadNodes">重新加载</a-button></div>
      <div v-else-if="!nodes.length" class="stream-state" role="status"><Server :size="36" /><strong>还没有可见媒体节点</strong></div>
      <div v-else-if="!selectedNodeId" class="stream-state" role="status"><Server :size="36" /><strong>请选择媒体节点</strong></div>
      <div v-else-if="loading && !pageData" class="stream-state" role="status" aria-label="正在加载媒体流"><a-spin />正在读取媒体流…</div>
      <div v-else-if="loadError && !pageData" class="stream-state stream-state--error" role="alert"><ShieldAlert :size="36" /><strong>{{ errorPresentation.label }}</strong><a-button v-if="errorPresentation.retryable" @click="refresh">重新加载</a-button></div>
        ...
        >
          <template #columns>
            <a-table-column title="媒体身份" :width="310">
              <template #cell="{ record }"><button type="button" class="identity-link" :aria-label="`查看流 ${record.media.app}/${record.media.stream} 详情`" @click="openDetail(record)"><Radio :size="14" /><span><strong>{{ record.media.app }}/{{ record.media.stream }}</strong><small>{{ record.media.schema }} · {{ record.media.vhost }}</small></span></button></template>
            </a-table-column>
            ...
            <a-table-column title="操作" :width="330" fixed="right">
              <template #cell="{ record }"><div class="row-actions"><a-button size="small" @click="openDetail(record)">详情</a-button><a-button v-if="canPreview" size="small" @click="openPreview(record)"><Eye :size="13" />预览</a-button><a-button v-if="canPreview" size="small" @click="openSnapshot(record)"><Camera :size="13" />截图</a-button><a-button v-if="canClose" size="small" status="danger" @click="openClose(record, false)">关闭</a-button><a-button v-if="canForceClose" size="small" status="danger" type="primary" @click="openClose(record, true)">强制</a-button></div></template>
            </a-table-column>
          </template>
      ...
      <a-drawer v-model:visible="previewVisible" :width="760" :footer="false" unmount-on-close @cancel="previewGeneration += 1; previewGrant = null">
        ...
        <div v-if="previewGrant" class="preview-expiry">授权到期：{{ previewGrant.expiresAt }}；地址仅来自后端授权响应，页面不持有节点 API Secret。</div>
        <PlayWindow :url="previewGrant?.url || ''" :zlm-webrtc="previewGrant?.protocol === 'webrtcs'" @error="Message.error($event)" />
      </a-drawer>

      <a-modal v-model:visible="snapshotVisible" modal-class="uvp-system-dialog" :width="860" :footer="false" unmount-on-close @cancel="clearSnapshot">
        <template #title>媒体截图</template>
        <div class="snapshot-body"><p>截图由共享鉴权客户端从 UVP 后端读取为 JPEG Blob，不在浏览器拼接 ZLM Host、端口、Secret 或 token 查询串。</p><div v-if="snapshotLoading" class="drawer-state"><a-spin />正在读取截图…</div><div v-else-if="snapshotError" class="preview-error" role="alert">{{ zlmErrorPresentation(snapshotError).label }}</div><img v-else-if="snapshotURL" :src="snapshotURL" alt="媒体流实时截图" /></div>
      </a-modal>

      <ZLMStreamCloseDialog v-model:visible="closeVisible" :node-name="selectedNodeName" :targets="closeTargets" :force="closeForce" :can-force="canForceClose" @done="closeDone" />
    </div>
  </div>
</template>

<style scoped>
.stream-management { box-sizing: border-box; height: 100%; padding: 4px 8px 24px; overflow: auto; color: var(--zlm-text-2); }
.stream-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 16px; margin-bottom: 16px; }.stream-header h1 { margin: 0; color: var(--zlm-text-1); font-size: 20px; }.stream-header p { margin: 5px 0 0; color: var(--zlm-text-3); font-size: var(--zlm-fs-caption); }.freshness { color: var(--zlm-text-3); font-size: var(--zlm-fs-caption); }.freshness--warning { color: var(--zlm-warn-600); }.freshness--danger { color: var(--zlm-danger-600); }
.stream-banner { margin-top: 12px; padding: 10px 14px; color: var(--zlm-text-2); background: var(--zlm-info-50); border: 1px solid var(--zlm-info-500); border-radius: var(--zlm-radius-md); font-size: var(--zlm-fs-caption); }.stream-banner--warning { color: var(--zlm-warn-600); background: var(--zlm-warn-50); border-color: var(--zlm-warn-500); }
.stream-search { margin: 16px 0; }.filter-short { width: 100px; }.filter-vhost { width: 170px; }.filter-app { width: 130px; }.filter-stream { width: 190px; }.filter-recording { width: 130px; }.selection-meta { color: var(--zlm-text-3); font-size: var(--zlm-fs-caption); }
.stream-state { display: flex; min-height: 300px; flex-direction: column; align-items: center; justify-content: center; gap: 10px; text-align: center; background: var(--uvp-panel-bg); border: 1px solid var(--uvp-panel-border); border-radius: var(--uvp-panel-radius); }.stream-state strong { color: var(--zlm-text-1); }.stream-state--error { color: var(--zlm-danger-600); background: var(--zlm-danger-50); border-color: var(--zlm-danger-500); }
.stream-table-panel { overflow: hidden; background: var(--uvp-panel-bg); border: 1px solid var(--uvp-panel-border); border-radius: var(--uvp-panel-radius); box-shadow: var(--uvp-panel-shadow); }.identity-link { display: inline-flex; align-items: center; gap: 8px; max-width: 100%; padding: 0; color: var(--zlm-brand-600); text-align: left; background: none; border: 0; cursor: pointer; }.identity-link > span { min-width: 0; }.identity-link strong, .identity-link small, .subline { display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }.identity-link small, .subline { margin-top: 2px; color: var(--zlm-text-4); font-family: var(--zlm-font-mono); font-size: 11px; }.identity-link:focus-visible { outline: 2px solid var(--zlm-brand-500); outline-offset: 3px; }.numeric { display: inline-flex; align-items: center; gap: 4px; font-family: var(--zlm-font-mono); }.recording { color: var(--zlm-danger-600); }.muted { color: var(--zlm-text-4); }.ownership { font-size: var(--zlm-fs-caption); }.ownership--managed { color: var(--zlm-success-600); }.ownership--owned, .ownership--conflicted { color: var(--zlm-warn-600); }.ownership--unknown { color: var(--zlm-danger-600); }.row-actions { display: flex; align-items: center; gap: 5px; }.row-actions svg { vertical-align: -2px; }
.stream-empty { display: flex; flex-direction: column; align-items: center; gap: 8px; padding: 48px 16px; color: var(--zlm-text-3); }.stream-empty strong { color: var(--zlm-text-1); }.stream-pagination { display: flex; align-items: center; justify-content: space-between; gap: 16px; padding: 12px 16px; color: var(--zlm-text-3); font-size: var(--zlm-fs-caption); border-top: 1px solid var(--zlm-border); }
.drawer-state { display: flex; min-height: 260px; flex-direction: column; align-items: center; justify-content: center; gap: 10px; }.drawer-state--error { color: var(--zlm-danger-600); }.detail-body { display: flex; flex-direction: column; gap: 18px; color: var(--zlm-text-2); }.detail-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 10px 16px; margin: 0; }.detail-grid div { min-width: 0; }.detail-grid dt { color: var(--zlm-text-3); font-size: var(--zlm-fs-caption); }.detail-grid dd { margin: 4px 0 0; overflow-wrap: anywhere; color: var(--zlm-text-1); font-family: var(--zlm-font-mono); }.detail-body h3 { margin: 0 0 8px; color: var(--zlm-text-1); font-size: 14px; }.detail-body p { color: var(--zlm-text-3); }.ownership-list { display: flex; flex-wrap: wrap; gap: 8px; }.ownership-list span { padding: 4px 8px; background: var(--zlm-fill-2); border: 1px solid var(--zlm-border); border-radius: var(--zlm-radius-sm); font-size: var(--zlm-fs-caption); }
.preview-toolbar { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 12px; }.preview-expiry, .preview-error { margin-bottom: 12px; padding: 9px 12px; color: var(--zlm-text-2); background: var(--zlm-info-50); border: 1px solid var(--zlm-info-500); border-radius: var(--zlm-radius-md); font-size: var(--zlm-fs-caption); }.preview-error { color: var(--zlm-danger-600); background: var(--zlm-danger-50); border-color: var(--zlm-danger-500); }.snapshot-body p { color: var(--zlm-text-3); font-size: var(--zlm-fs-caption); }.snapshot-body img { display: block; max-width: 100%; max-height: 70vh; margin: 0 auto; background: #020617; border-radius: var(--zlm-radius-md); }
@media (max-width: 900px) { .stream-header { flex-direction: column; }.filter-short, .filter-vhost, .filter-app, .filter-stream, .filter-recording { width: 100%; }.detail-grid { grid-template-columns: 1fr; } }
</style>
