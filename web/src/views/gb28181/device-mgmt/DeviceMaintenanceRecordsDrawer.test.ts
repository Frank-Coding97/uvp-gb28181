    ...
    global: {
      stubs: {
        ...
        "a-button": {
          ...
          template:
            "<button :disabled='disabled' :aria-busy='loading' @click='$emit(`click`)'><slot name='icon' /><slot /></button>"
        },
        "a-pagination": {
...
describe("DeviceMaintenanceRecordsDrawer", () => {
  ...
  it("shows real timestamps, reported firmware, failure reason, and collapsed technical data", async () => {
    api.listFirmwareUpgrades.mockResolvedValue(
      page([
        upgradeOperation({
          status: "failed",
  ...
  it("ignores stale responses after switching devices", async () => {
    ...
    api.listMaintenanceOperations.mockImplementationOnce(
      () =>
        new Promise(resolve => {
          resolveOld = resolve;
        })
