...
type SelectedRecord = { type: "reboot"; item: MaintenanceOperation } | { type: "upgrade"; item: UpgradeOperation };
...
function isCurrent(type: RecordType, token: number, version: number, deviceId: number) {
  return token === contextVersion && requestVersions[type] === version && props.visible && props.device?.id === deviceId;
}
...
function rebootSummary(operation: MaintenanceOperation) {
  ...
  // ⛔ 别说"设备执行结果未回传"（2026-09-20 改）：否定式读起来像"该收的没收到"，
  //    用户会以为下发失败。重启命令协议里就没有执行结果回执 ⇒ 只陈述"已下发"这个事实。
  if (key === "sent") return "平台已下发指令（该命令无需设备回执）";
...
async function loadPage(type: RecordType, page: number, token = contextVersion, deviceId = props.device?.id) {
  ...
  try {
    ...
    if (isCurrent(type, token, version, deviceId))
      state.error = errorText(error, type === "reboot" ? "重启记录加载失败" : "升级记录加载失败");
  } finally {
  ...
  >
    <template #title>
      <div class="records-title">
        ...
        <div>
          <strong>维护记录</strong><span>{{ deviceName }}</span>
        </div>
      </div>
    </template>
      ...
      <template v-else>
        <section class="records-device-summary" aria-label="当前设备">
          ...
          <dl>
            ...
            <div>
              <dt>当前固件</dt>
              <dd class="mono">{{ device.firmware || "未上报" }}</dd>
            </div>
            <div>
              <dt>通道在线</dt>
              <dd>{{ device.channelOnlineCount }} / {{ device.channelCount }}</dd>
            </div>
          </dl>
        </section>
        ...
        <div class="records-toolbar">
          ...
          <a-button
            data-testid="maintenance-records-refresh"
            class="uvp-refresh-btn"
            size="small"
            :loading="activeState.loading"
            @click="refreshCurrent"
            ><RefreshCcw :size="14" />刷新</a-button
          >
        </div>

        <div v-if="selectedRecord" class="record-detail" data-testid="maintenance-record-detail">
          <p v-if="activeState.error" class="records-error" role="alert">
            刷新未完成，当前显示上次读取的结果。{{ activeState.error }}
          </p>
          <button type="button" class="detail-back" data-testid="maintenance-record-detail-back" @click="closeDetails">
            <ArrowLeft :size="15" />返回记录
          </button>
          <header class="detail-heading">
            <div>
              <h2>{{ operationLabel(selectedRecord.type) }}</h2>
            </div>
            <span class="record-status" :class="`tone-${selectedStatusTone(selectedRecord)}`"
              ><CircleCheck v-if="selectedStatusTone(selectedRecord) === 'success'" :size="14" /><Clock3
                v-else-if="selectedStatusTone(selectedRecord) === 'pending'"
                :size="14"
              /><CircleX v-else :size="14" />{{ selectedStatusText(selectedRecord) }}</span
            >
          </header>
          ...
          <dl class="detail-facts">
            ...
            <div>
              <dt>摘要</dt>
              <dd>{{ selectedSummary(selectedRecord) }}</dd>
            </div>
            <div v-if="selectedRecord.type === 'upgrade'">
              <dt>目标版本</dt>
              <dd class="mono">{{ selectedVersion(selectedRecord) }}</dd>
            </div>
            <div v-if="selectedRecord.type === 'upgrade'">
              <dt>设备回报版本</dt>
              <dd class="mono">{{ selectedReportedVersion(selectedRecord) }}</dd>
            </div>
            <div v-if="['failed', 'rejected', 'cancelled'].includes(selectedRecord.item.status)">
              <dt>失败原因</dt>
              <dd>{{ failureReason(selectedRecord) }}</dd>
            </div>
          </dl>
          ...
          <section class="detail-section">
            ...
            <ol class="detail-timeline">
              <li v-for="node in selectedTimeline" :key="node.label">
                <span class="timeline-dot"></span>
                <div>
                  <strong>{{ node.label }}</strong
                  ><time>{{ formatDateTime(node.value) }}</time>
                </div>
              </li>
            </ol>
          </section>
          ...
          <details class="technical-details" data-testid="maintenance-record-technical">
            ...
            <dl>
              <div v-for="entry in selectedTechnical" :key="entry.label">
                <dt>{{ entry.label }}</dt>
...
.record-row {
  display: grid;
  grid-template-columns: minmax(125px, 1.1fr) minmax(85px, 0.8fr) minmax(76px, 0.7fr) minmax(66px, 0.6fr) minmax(130px, 1.4fr);
  gap: 10px;
  align-items: center;
  width: 100%;
  padding: 11px 12px;
...
.record-row-item:hover {
  background: var(--uvp-brand-soft);
}
.record-row-item time {
  font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
  font-size: 11px;
  color: var(--uvp-text-tertiary);
}
.record-row-item strong {
  overflow: hidden;
  text-overflow: ellipsis;
  color: var(--uvp-text-primary);
  white-space: nowrap;
...
.record-status.tone-success {
  color: var(--uvp-success);
}
.record-status.tone-pending {
  color: var(--uvp-warning);
}
.record-status.tone-warning {
  color: var(--uvp-warning);
}
.record-status.tone-danger {
  color: var(--uvp-danger);
}
.records-state,
.records-empty {
...
.timeline-dot {
  ...
  margin-top: 4px;
  background: var(--uvp-brand);
  border: 2px solid var(--uvp-panel-bg);
  border-radius: 50%;
  box-shadow: 0 0 0 1px var(--uvp-brand);
}
.detail-timeline li > div {
  display: grid;
  gap: 2px;
}
...
.spin {
  animation: maintenance-record-spin 1s linear infinite;
}

@keyframes maintenance-record-spin {
  to {
...
@media (width <= 600px) {
  ...
  .records-device-summary dl div:last-child {
    grid-column: 1 / -1;
  }
  .records-toolbar {
    align-items: flex-start;
