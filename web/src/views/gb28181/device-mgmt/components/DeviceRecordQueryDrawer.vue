...
const contextChannelName = computed(
  () => options.value?.channel.name || props.channel?.alias || props.channel?.name || "当前通道"
);
const stateMeta = computed(
  () =>
    ({
      ...
      querying: {
        label: "正在查询设备",
        detail: `设备可能需要 ${options.value?.timeoutSeconds || 15} 秒返回录像目录`,
        tone: "info",
        icon: Loader2
      },
      complete: {
        label: "查询完成",
        detail: `已收到 ${result.value?.receivedCount || 0} 条完整录像目录`,
        tone: "success",
        icon: CheckCircle2
      },
      ...
      partial: {
        label: "结果可能不完整",
        detail: `设备声明 ${result.value?.declaredTotal || 0} 条，当前收到 ${result.value?.receivedCount || 0} 条`,
        tone: "warning",
        icon: AlertTriangle
      },
      timeout: {
        label: "查询超时",
        detail: errorMessage.value || "设备未返回有效的录像目录，可保持条件重试",
        tone: "danger",
        icon: Clock3
      },
      offline: {
        label: "设备离线",
        detail: errorMessage.value || "所属设备当前离线，无法查询录像目录",
        tone: "danger",
        icon: WifiOff
      },
      error: {
        label: "查询失败",
        detail: errorMessage.value || "录像查询未完成，请稍后重试",
        tone: "danger",
        icon: AlertTriangle
      }
    })[state.value]
      ...
      <div class="record-query-layout" data-testid="record-query-drawer">
        <section v-if="options" class="context-strip" aria-label="当前查询对象">
          <div class="context-main">
            ...
            <div>
              <span>所属设备</span><strong>{{ options.device.name }}</strong
              ><code>{{ options.device.code }}</code>
            </div>
          </div>
          <div class="context-main">
            <span class="context-icon channel"><Video :size="17" /></span>
            <div>
              <span>查询通道</span><strong>{{ options.channel.name }}</strong
              ><code>{{ options.channel.code }}</code>
            </div>
          </div>
          <div class="context-meta">
            <span>平台时区</span><strong>{{ options.timezone }}</strong>
          </div>
          <div class="context-meta">
            <span>最长跨度</span><strong>{{ options.maxRangeHours }} 小时</strong>
          </div>
        </section>
        ...
        <section class="query-section" aria-labelledby="record-query-condition-title">
          <div class="section-heading">
            <div>
              <strong id="record-query-condition-title">查询条件</strong
              ><span>时间按平台时区 {{ options?.timezone || "-" }} 发送</span>
            </div>
          ...
          <div class="query-form">
            <label class="field range-field">
              ...
              />
              <small v-if="errors.startTime || errors.endTime" class="field-error">{{
                errors.startTime || errors.endTime
              }}</small>
              <small v-else class="field-value"
            ...
            <div class="query-actions">
              ...
              >
                <template #icon
                  ><RefreshCw v-if="result || state === 'timeout' || state === 'error'" :size="15" /><Search v-else :size="15"
                /></template>
          ...
          <div v-if="advancedVisible" class="advanced-fields">
            ...
            >
            <label class="field"
              ><span>录像机标识</span><a-input v-model="form.recorderId" allow-clear placeholder="设备有明确要求时填写"
            /></label>
          </div>
        </section>
        ...
        <section class="result-section" aria-labelledby="record-query-result-title">
          <div class="section-heading result-heading">
            <div>
              <strong id="record-query-result-title">查询结果</strong
              ><span v-if="result">耗时 {{ (result.elapsedMs / 1000).toFixed(1) }} 秒 · 本地分页</span>
            </div>
            <div v-if="result" class="result-counts">
              <span
                >设备声明 <b>{{ result.declaredTotal }}</b></span
              ><span
                >平台收到 <b>{{ result.receivedCount }}</b></span
              >
            </div>
          </div>
          ...
          <div v-if="hasResults" class="result-table-wrap">
            <a-table
              ...
              class="record-result-table"
            >
              <template #columns>
                <a-table-column title="录像名称" :width="190"
                  ><template #cell="{ record }"
                    ><a-tooltip :content="record.name || '未知'"
                      ><span class="ellipsis">{{ record.name || "未知" }}</span></a-tooltip
                    ></template
                  ></a-table-column
                >
                <a-table-column title="开始时间" :width="170"
                  ><template #cell="{ record }"
                    ><span class="mono">{{ displayDateTime(record.startTime) }}</span></template
                  ></a-table-column
                >
                <a-table-column title="结束时间" :width="170"
                  ><template #cell="{ record }"
                    ><span class="mono">{{ displayDateTime(record.endTime) }}</span></template
                  ></a-table-column
                >
                <a-table-column title="时长" :width="100"
                  ><template #cell="{ record }">{{ durationText(record.startTime, record.endTime) }}</template></a-table-column
                >
                <a-table-column title="类型" :width="100"
                  ><template #cell="{ record }"
                    ><span class="type-badge">{{ recordQueryTypeText(record.type) }}</span></template
                  ></a-table-column
                >
                <a-table-column title="大小" :width="100"
                  ><template #cell="{ record }">{{ fileSizeText(record.fileSize) }}</template></a-table-column
                >
                <a-table-column title="文件路径" :width="230"
                  ><template #cell="{ record }"
                    ><a-tooltip :content="record.filePath || '未知'"
                      ><code class="ellipsis">{{ record.filePath || "未知" }}</code></a-tooltip
                    ></template
                  ></a-table-column
                >
                <a-table-column title="存储位置" :width="210"
                  ><template #cell="{ record }"
                    ><a-tooltip :content="record.recordLocation || '未知'"
                      ><code class="ellipsis">{{ record.recordLocation || "未知" }}</code></a-tooltip
                    ></template
                  ></a-table-column
                >
              </template>
...
.drawer-title-icon {
  ...
  background: color-mix(in srgb, var(--uvp-primary) 12%, transparent);
  border-radius: 6px;
}
.drawer-title > div:last-child {
  display: flex;
  flex-direction: column;
  min-width: 0;
}
...
.context-strip {
  display: grid;
  grid-template-columns: minmax(210px, 1fr) minmax(210px, 1fr) 140px 110px;
  gap: 18px;
  align-items: center;
  padding: 14px 18px;
...
.context-icon.channel {
  color: var(--uvp-success);
  background: color-mix(in srgb, var(--uvp-success) 10%, transparent);
}
.context-main > div,
.context-meta {
  display: flex;
...
.advanced-toggle:hover {
  color: var(--uvp-primary);
  background: color-mix(in srgb, var(--uvp-primary) 8%, transparent);
}
...
.query-form {
  display: grid;
  grid-template-columns: minmax(390px, 1fr) 170px auto;
  gap: 14px;
  align-items: end;
}
.advanced-fields {
...
.tone-info {
  color: var(--uvp-primary);
  background: color-mix(in srgb, var(--uvp-primary) 7%, var(--uvp-bg));
  border-left-color: var(--uvp-primary);
}
.tone-success {
  color: var(--uvp-success);
  background: color-mix(in srgb, var(--uvp-success) 7%, var(--uvp-bg));
  border-left-color: var(--uvp-success);
}
.tone-warning {
  color: var(--uvp-warning);
  background: color-mix(in srgb, var(--uvp-warning) 8%, var(--uvp-bg));
  border-left-color: var(--uvp-warning);
}
.tone-danger {
  color: var(--uvp-danger);
  background: color-mix(in srgb, var(--uvp-danger) 7%, var(--uvp-bg));
  border-left-color: var(--uvp-danger);
}
.result-table-wrap {
  overflow-x: auto;
  border: 1px solid var(--uvp-border);
...
.type-badge {
  ...
  padding: 0 7px;
  color: var(--uvp-text-secondary);
  background: var(--uvp-bg-tertiary);
  border-radius: 4px;
}
.result-placeholder {
  display: flex;
  ...
  min-height: 190px;
  color: var(--uvp-text-tertiary);
  border: 1px dashed var(--uvp-border);
  border-radius: 5px;
}
.result-pagination {
  display: flex;
...
.spin {
  animation: record-query-spin 1s linear infinite;
}

@keyframes record-query-spin {
  to {
