...
const channel = (id = 31): ChannelVO => ({
  ...
  parentId: "",
  ptzType: 0,
  // 设备上报的通道属性(GB/T 28181 附录 A):本用例只关心录像查询入口,
  // 全部按"设备未上报"构造,不参与断言。
  roomType: 0,
  supplyLightType: 0,
  directionType: 0,
...
describe("DeviceRecordQueryDrawer", () => {
  ...
  it.each([
    ...
    ["empty", "未查询到录像"],
    ["partial", "结果可能不完整"]
  ] as Array<[RecordQueryResult["status"], string]>)("renders the %s result state", async (status, expectedText) => {
  ...
  it("reloads a new channel and ignores the stale options response", async () => {
    ...
    api.getRecordQueryOptions.mockImplementationOnce(
      () =>
        new Promise(resolve => {
          resolveOld = resolve;
        })
