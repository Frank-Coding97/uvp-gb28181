...
vi.mock("@visactor/vchart", () => ({
  default: class {
    ...
    on(_type: string, _query: Record<string, any>, handler: (params: Record<string, any>) => void) {
      vchart.events.push(handler);
    }
  }
}));
...
const viewerTableStub = {
  ...
  provide() {
    return { viewerTable: this };
  },
  template:
    "<div class='viewer-table-stub' :data-loading='String(loading)'><slot name='columns' /><template v-for='record in data'><slot v-if='expandedKeys?.includes(record.channelId)' name='expand-row' :record='record' /></template><slot v-if='!data.length' name='empty' /></div>"
};
const viewerColumnStub = {
  props: ["title", "dataIndex"],
  inject: ["viewerTable"],
  template:
    "<div>{{ title }}<template v-for='record in viewerTable.data'><span v-if='dataIndex'>{{ record[dataIndex] }}</span><slot v-else name='cell' :record='record' /></template></div>"
};
...
describe("runtime monitor components", () => {
  ...
  it("renders daily traffic as stacked columns with a total line", async () => {
    trafficApi.getTrafficSummary.mockResolvedValue(
      ok({
        upstreamBytes: 2048,
    ...
    trafficApi.getTrafficTrend.mockResolvedValue(
      ok({
  ...
  it("switches to real hourly buckets and opens the paged ledger from a bar", async () => {
    trafficApi.getTrafficSummary.mockResolvedValue(
      ok({
        ...
        upstreamSessions: 0,
        downstreamSessions: 0,
        from: "2026-08-20T16:00:00+08:00",
    ...
    trafficApi.getTrafficTrend.mockResolvedValue(
      ok({
        list: [
          {
            bucket: "2026-08-21T15:00:00+08:00",
    ...
    await flushPromises();
    expect(trafficApi.getTrafficSessions).toHaveBeenCalledWith(
  ...
  it("does not render kick actions when the backend denies super-admin capability", async () => {
    trafficApi.getCurrentViewers.mockResolvedValue(
      ok({
        list: [
          {
            channelId: "channel-1",
    ...
    expect(trafficApi.getCurrentViewers).toHaveBeenCalledWith(
      { deviceId: "device-1", channelId: undefined },
      expect.any(AbortSignal)
    );
...
describe("runtime monitor entry contract", () => {
  ...
  it("keeps device monitoring lazy, switchable and explicit about its state", () => {
    ...
    // ⛔ 用正则而不是整串 `toContain`：prettier 会把多属性标签拆成**多行**
    //   （`<ViewerTable\n  v-if=…`），提交钩子跑过 prettier --write 之后
    //   整串断言就会因为"只是换了行"而红。
    expect(source).toMatch(/<ViewerTable\s+v-if="runtimeActiveTab === 'viewers'"/);
