...
describe("SIP platform deployment card", () => {
    it("uses the shared page-action language in the top toolbar", () => {
        expect(source).toContain('<a-button v-if="config" class="uvp-page-action-btn" @click="copyAll">');
        expect(source).toContain('class="uvp-page-action-btn uvp-refresh-btn"');
