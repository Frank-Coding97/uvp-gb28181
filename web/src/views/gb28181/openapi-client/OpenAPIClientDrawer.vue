...
const form = reactive({ name: "", ownerDeptId: 0, responsibleUserId: "" });
...
watch(() => props.scopes, () => {
  if (props.visible && props.mode === "detail") selectedScopes.value = props.scopes.filter(item => item.enabled).map(item => item.scope);
}, { deep: true });
  ...
  >
    ...
    <template v-if="props.mode === 'create'">
      <a-form layout="vertical" class="openapi-client-drawer__form">
        <a-form-item label="客户端名称" required :validate-status="touched.name && !form.name.trim() ? 'error' : ''" :help="touched.name && !form.name.trim() ? '请输入客户端名称' : ''">
          <a-input v-model="form.name" allow-clear maxlength="100" placeholder="例如：现场集成服务" @blur="touched.name = true" />
        </a-form-item>
        <a-form-item label="归属部门" required :validate-status="touched.ownerDeptId && !form.ownerDeptId ? 'error' : ''" :help="touched.ownerDeptId && !form.ownerDeptId ? '请选择归属部门' : ''">
          <a-select v-model="form.ownerDeptId" allow-clear placeholder="请选择当前可管理部门" @blur="touched.ownerDeptId = true">
            <a-option v-for="department in props.departments" :key="department.id" :value="department.id">
              {{ department.name }}
            </a-option>
          </a-select>
        </a-form-item>
      ...
      <a-alert type="info">客户端只访问归属部门的设备；不包含下级部门，也不包含共享可见设备。创建成功后能力默认为空。</a-alert>
    </template>
    ...
    <template v-else>
      <a-descriptions v-if="props.client" :column="1" bordered class="openapi-client-drawer__summary">
        ...
        <a-descriptions-item label="AK"><code>{{ props.client.ak }}</code></a-descriptions-item>
        <a-descriptions-item label="归属部门">{{ props.departments.find(item => item.id === props.client?.ownerDeptId)?.name || `部门 #${props.client.ownerDeptId}` }}</a-descriptions-item>
        ...
        <a-descriptions-item label="rowVersion"><code>{{ props.client.rowVersion }}</code></a-descriptions-item>
      </a-descriptions>
      ...
      <div class="openapi-client-drawer__revocation">
        ...
        <div>
          ...
          <a-tag v-if="props.revocationStatus" :color="props.revocationStatus.status === 'closed' ? 'green' : 'orange'">
            {{ props.revocationStatus.status === "closed" ? "已确认清退" : props.revocationStatus.status === "pending" ? "清退中" : "状态未知" }}
          </a-tag>
          <span v-else class="openapi-client-drawer__muted">尚未查询</span>
          <span v-if="props.revocationStatus" class="openapi-client-drawer__counts">待清退 {{ props.revocationStatus.pending }}，已关闭 {{ props.revocationStatus.closed }}</span>
        </div>
        <div v-if="props.revocationError" class="openapi-client-drawer__warning" role="alert">{{ props.revocationError }}</div>
        <a-button v-if="props.canStatus" size="small" :loading="props.revocationLoading" @click="emit('refreshRevocation')">刷新清退进度</a-button>
      </div>

      <a-divider>调用审计</a-divider>
      <a-button v-if="props.canAudit" size="small" :loading="props.auditLoading" @click="emit('loadAudits')">加载最近审计</a-button>
      <a-empty v-if="props.canAudit && !props.auditLoading && props.auditItems.length === 0" description="暂无审计记录或尚未加载" />
      <div v-if="props.auditItems.length" class="openapi-client-drawer__audits">
        <div v-for="item in props.auditItems" :key="`${item.requestId}-${item.createdAt}`" class="openapi-client-drawer__audit-row">
    ...
    <template #footer>
      ...
      <a-button v-if="props.mode === 'create' || props.canGrant" type="primary" :loading="props.submitting" :disabled="!canSubmit" @click="submit">
