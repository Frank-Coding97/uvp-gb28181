  ...
  <div class="dashboard-shell">
    <header class="dashboard-header">
      ...
      <div class="actions">
        ...
        <button v-if="!editing" class="btn-primary primary" @click="startEditing"><Pencil :size="15" />编辑仪表盘</button>
        <template v-else>
          ...
          <button class="btn-primary primary" :disabled="saving" @click="saveLayout"><Save :size="15" />{{ saving ? "保存中" : "保存布局" }}</button>
        </template>
      </div>
    </header>
    ...
    />
    <MediaRuntimeLedgerDialog :visible="mediaLedgerVisible" :kind="mediaLedgerKind" :ledger="mediaRuntimeLedger" @close="mediaLedgerVisible = false" @kind="mediaLedgerKind = $event" />
  </div>
</template>
...
const mediaRuntimeLedger = computed(() => buildMediaRuntimeLedger(mediaOverview.value));
