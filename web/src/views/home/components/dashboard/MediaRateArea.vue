<template>
  <div class="media-rate-area">
    ...
    <div class="media-rate-area__axis"><span>最近 2 分钟</span><span>现在</span></div>
  </div>
</template>
...
<style scoped>
.media-rate-area{display:flex;flex-direction:column;height:calc(100% - 74px);min-height:150px}.media-rate-area svg{width:100%;height:100%;overflow:visible}.media-rate-area__grid{fill:none;stroke:var(--uvp-panel-border);stroke-width:.45;vector-effect:non-scaling-stroke}.media-rate-area__fill,.media-rate-area__line{pointer-events:none}.media-rate-area__line{fill:none;stroke-width:1.8;stroke-linecap:round;stroke-linejoin:round;vector-effect:non-scaling-stroke}.media-rate-area__point{stroke:var(--uvp-panel-bg);stroke-width:1;vector-effect:non-scaling-stroke}.media-rate-area__axis{display:flex;justify-content:space-between;margin-top:5px;font-size:10px;color:var(--uvp-text-tertiary)}
</style>
