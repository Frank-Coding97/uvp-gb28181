...
const summary = computed(() => {
  ...
  if (play.value) return `终态样本 ${formatNumber(play.value.summary.attempts)}，成功 ${formatNumber(play.value.summary.success)}，失败 ${formatNumber(play.value.summary.failure)}`;
  if (traffic.value) return `上行 ${formatBytes(traffic.value.summary.upstreamBytes)}，下行 ${formatBytes(traffic.value.summary.downstreamBytes)}`;
  return "暂无下钻数据";
});
...
<template>
  <a-modal :visible="visible" modal-class="uvp-system-dialog" :title="title" :width="980" :footer="false" unmount-on-close @cancel="emit('close')">
    <div class="drilldown-dialog" :aria-busy="loading">
      ...
      <p v-if="result?.coverage === 'partial'" class="drilldown-notice" role="status">统计覆盖不完整，图中断点表示该时段数据未知。</p>
      <MediaVChart
        title="趋势"
        :spec="chartSpec"
        :status="chartStatus"
      ...
      <section v-else-if="play" class="ledger-section">
        <h3>结果分布</h3>
        <div class="summary-grid"><span><small>成功率</small><strong>{{ formatPercent(play.summary.rate) }}</strong></span><span><small>进行中</small><strong>{{ formatNumber(play.summary.started) }}</strong></span><span v-for="row in play.failureStages" :key="row.key"><small>失败 · {{ row.key }}</small><strong>{{ formatNumber(row.count) }}</strong></span></div>
      </section>
      <section v-else-if="traffic" class="ledger-section">
        <h3>设备 / 通道台账（{{ formatNumber(traffic.ledger.total) }}）</h3>
        <div class="ledger-scroll"><table><thead><tr><th>设备</th><th>通道</th><th>上行</th><th>下行</th></tr></thead><tbody>
          <tr v-for="row in traffic.ledger.rows" :key="`${row.deviceCode}/${row.channelCode}`"><td>{{ row.deviceCode }}</td><td>{{ row.channelCode || '--' }}</td><td>{{ formatBytes(row.upstreamBytes) }}</td><td>{{ formatBytes(row.downstreamBytes) }}</td></tr>
        </tbody></table></div>
      </section>
    </div>
  </a-modal>
...
@media(max-width:600px){.drilldown-dialog{max-height:80vh}.drilldown-toolbar{align-items:flex-start;flex-direction:column}.drilldown-ranges{width:100%}.drilldown-ranges button{flex:1;padding-inline:5px}.summary-grid{grid-template-columns:1fr 1fr}}
</style>
