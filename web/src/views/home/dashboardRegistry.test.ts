...
describe("dashboard registry", () => {
  it("contains five equal visible cards in the reference first row", () => {
    ...
    for (const [index, id] of ["sip-rpm", "sip-today", "play-success-24h", "media-traffic-today", "media-runtime"].entries()) {
      expect(DEFAULT_DASHBOARD_LAYOUT.widgets.find(widget => widget.id === id)).toMatchObject({ x: index * 4, y: 0, w: 4, h: 3, visible: true });
    }
    expect(DEFAULT_DASHBOARD_LAYOUT.widgets.find(widget => widget.id === "sip-monitor")?.visible).toBe(false);
    expect(DEFAULT_DASHBOARD_LAYOUT.widgets.find(widget => widget.id === "media-rate")).toMatchObject({ x: 0, y: 3, w: 12, h: 5, visible: true });
    expect(DEFAULT_DASHBOARD_LAYOUT.widgets.find(widget => widget.id === "device-online-rate")).toMatchObject({ x: 12, y: 3, w: 4, h: 5, visible: true });
    expect(DEFAULT_DASHBOARD_LAYOUT.widgets.find(widget => widget.id === "channel-online-rate")).toMatchObject({ x: 16, y: 3, w: 4, h: 5, visible: true });
  });
  ...
  it("migrates the legacy first row without preserving its hidden-card gap", () => {
    ...
    expect(normalized.widgets.find(widget => widget.id === "device-online-rate")).toMatchObject({ x: 12, y: 3, w: 4, h: 5 });
    expect(normalized.widgets.find(widget => widget.id === "channel-online-rate")).toMatchObject({ x: 16, y: 3, w: 4, h: 5 });
  });
  ...
  it("migrates the schema 2 default layout to the new first row", () => {
    ...
    for (const [index, id] of ["sip-rpm", "sip-today", "play-success-24h", "media-traffic-today", "media-runtime"].entries()) {
      expect(normalized.widgets.find(widget => widget.id === id)).toMatchObject({ x: index * 4, y: 0, w: 4, h: 3, visible: true });
    }
    expect(normalized.widgets.find(widget => widget.id === "sip-monitor")?.visible).toBe(false);
    expect(normalized.widgets.find(widget => widget.id === "device-online-rate")).toMatchObject({ x: 12, y: 3, w: 4, h: 5 });
    expect(normalized.widgets.find(widget => widget.id === "channel-online-rate")).toMatchObject({ x: 16, y: 3, w: 4, h: 5 });
  ...
  it("rejects unknown, duplicate or invalid widgets", () => {
    for (const widgets of invalidLayouts) {
      expect(() => normalizeDashboardLayout({ schemaVersion: 3, widgets })).toThrow();
    }
  });
});
