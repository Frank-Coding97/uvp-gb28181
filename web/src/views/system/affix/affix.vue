<template>
  <div class="snow-fill affix-page">
    <div class="snow-fill-inner uvp-page-shell-flat container">
      <s-layout-search>
        <template #fields>
          <a-input v-model="form.name" placeholder="请输入文件名" style="width: 176px" allow-clear @press-enter="search" />
          <a-select placeholder="文件类型" v-model="form.ftype" style="width: 126px" allow-clear>
      ...
      >
        <template #columns>
          <a-table-column title="文件名" data-index="name" :ellipsis="true" tooltip :width="200"></a-table-column>
          ...
          <a-table-column title="后缀" data-index="suffix" :width="70" :ellipsis="true" tooltip></a-table-column>
          ...
          <a-table-column title="创建人" data-index="createdBy" :width="150">
            <template #cell="{ record }">{{ record.user ? record.user.nickName : "" }}</template>
          </a-table-column>
            ...
            <template #cell="{ record }">
              <div class="uvp-table-actions">
                <a-link
                  ...
                  v-hasPerm="['system:affix:download']"
                >
                  <template #icon><icon-download /></template>
                  下载
                </a-link>
                <a-link
                  class="uvp-table-action uvp-table-action--copy"
                  @click="copyLink(record)"
                  v-hasPerm="['system:affix:copy']"
                >
                ...
                <a-link
                  ...
                  @click="onEdit(record)"
                  v-hasPerm="['system:affix:updateName']"
                >
                  <template #icon><icon-edit /></template>
                  重命名
                </a-link>
                <a-popconfirm type="warning" content="确定删除该文件吗?" @ok="onDelete(record)">
                  <a-link class="uvp-table-action uvp-table-action--delete" v-hasPerm="['system:affix:delete']">
    ...
    <a-modal
      modal-class="uvp-system-dialog"
      v-model:visible="chunkUploadVisible"
      title="大文件上传"
      :footer="false"
      :width="700"
      @close="onChunkUploadClose"
    >
      <chunk-upload ref="chunkUploadRef" :auto-start="true" @success="onChunkUploadSuccess" />
    </a-modal>
  </div>
</template>

<script setup lang="ts">
import { getAffixListAPI, deleteAffixAPI, updateAffixNameAPI, uploadAffixAPI, downloadAffixAPI } from "@/api/file";
