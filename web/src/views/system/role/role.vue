    ...
    <div class="snow-inner uvp-page-shell-flat">
      <s-layout-search>
        <template #fields>
          ...
          <a-select placeholder="角色状态" v-model="form.status" style="width: 126px" allow-clear>
          ...
          <a-table-column title="操作" :width="300" align="center" :fixed="isMobile ? '' : 'right'">
            <template #cell="{ record }">
              <div class="uvp-table-actions">
                ...
                <a-link
                  class="uvp-table-action uvp-table-action--scope"
                ...
                <a-popconfirm type="warning" content="确定删除该角色吗?" @ok="onDelete(record)">
                  <a-link class="uvp-table-action uvp-table-action--delete" v-hasPerm="['system:role:delete']">
...
const tableScroll = computed(() => ({
  x: "100%",
  minWidth: 1000,
  ...(displayRoleList.value.length > 0 ? { y: "100%" } : {})
}));

// 查询功能 - 在前端完成筛选，递归查找
const search = () => {
  const nameFilter = form.value.name?.trim();
  const hasStatusFilter = form.value.status !== null && form.value.status !== undefined && form.value.status !== "";
  const statusFilter = hasStatusFilter ? Number(form.value.status) : null;
  // 如果没有筛选条件，显示所有数据
  if (!nameFilter && !hasStatusFilter) {
...
const onDataScope = (row: any) => {
  ...
  const defaultCheckedKeys = String(row.checkedDepts ?? "")
    .split(",")
    .map(item => Number(item.trim()))
    .filter(key => Number.isInteger(key) && key > 0);
  Modal.open({
    title: "数据权限",
    modalClass: "uvp-system-dialog uvp-data-scope-dialog",
    width: dataScopeDialogWidth.value,
    okText: "确定",
    cancelText: "取消",
    content: () =>
      h(DataScope, {
    ...
    onBeforeOk: async () => {
      ...
      if (dataScopeRef.value) {
        ...
        //console.log(formData)
        try {
          const checkedKeys = Array.isArray(formData.checkedKeys)
            ? formData.checkedKeys.map((key: unknown) => Number(key)).filter(key => Number.isInteger(key) && key > 0)
            : [];
          await editDataScopeAPI({
            id: row.id,
            dataScope: Number(formData.dataScope),
            checkedDepts: checkedKeys.join(",")
          });
