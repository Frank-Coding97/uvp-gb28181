<template>
  <div class="snow-page">
    <div class="snow-inner">
      <s-layout-search>
        <template #fields>
          <a-input v-model="form.title" placeholder="请输入API标题" style="width: 176px" allow-clear @press-enter="onSearch" />
          <a-input v-model="form.path" placeholder="请输入API路径" style="width: 176px" allow-clear @press-enter="onSearch" />
      ...
      >
        <template #columns>
          ...
          <a-table-column title="关联的菜单" :width="170" ellipsis tooltip>
            <template #cell="{ record }">
              {{ record.sysMenuList ? record.sysMenuList.map((item: any) => item.id + "：" + item.title).join(", ") : "无" }}
            </template>
          </a-table-column>
          <a-table-column title="创建时间" data-index="createdAt" :width="180">
            <template #cell="{ record }">{{ record.createdAt ? formatTime(record.createdAt) : "" }}</template>
          </a-table-column>
          <a-table-column title="操作" :width="160" align="center" :fixed="isMobile ? '' : 'right'">
            <template #cell="{ record }">
              <div class="uvp-table-actions">
                <a-link class="uvp-table-action uvp-table-action--edit" @click="onUpdate(record)" v-hasPerm="['system:api:edit']">
                  <template #icon><icon-edit /></template>
                  <span>修改</span>
                </a-link>
                <a-popconfirm type="warning" content="确定删除该项吗?" @ok="onDelete(record)">
                  <a-link class="uvp-table-action uvp-table-action--delete" v-hasPerm="['system:api:delete']">
...
const tableScroll = computed(() => ({
  x: "100%",
  minWidth: 1000,
  ...(tableData.value.length > 0 ? { y: "100%" } : {})
}));
