            ...
            <a-row :gutter="24">
              ...
              <a-col :span="12">
                <a-form-item label="是否覆盖" field="isCover">
                  <a-checkbox :model-value="Boolean(editForm.isCover)" @update:model-value="editForm.isCover = $event ? 1 : 0">
                    是否覆盖已存在的文件
                  </a-checkbox>
                </a-form-item>
              </a-col>
              <a-col :span="12">
                <a-form-item label="是否生成菜单" field="isMenu">
                  <a-checkbox :model-value="Boolean(editForm.isMenu)" @update:model-value="editForm.isMenu = $event ? 1 : 0">
                    是否生成菜单
                  </a-checkbox>
                </a-form-item>
              </a-col>
              <a-col :span="12">
                <a-form-item label="是否生成树形数据" field="isTree">
                  <a-checkbox :model-value="Boolean(editForm.isTree)" @update:model-value="handleIsTreeChange">
                    是否生成树形数据
                  </a-checkbox>
                  <template #extra> 数据库中须包含id（主键）、name及parent_id字段，且parent_id与主键数据类型需一致。 </template>
                </a-form-item>
              </a-col>
              <a-col :span="24">
                <a-form-item label="是否关联树形数据" field="isRelationTree">
                  <a-checkbox :model-value="Boolean(editForm.isRelationTree)" @update:model-value="handleIsRelationTreeChange">
                    是否关联树形数据
                  </a-checkbox>
                </a-form-item>
              </a-col>
              <a-col :span="12" v-if="editForm.isRelationTree === 1">
                <a-form-item label="关联树形数据表" field="relationTreeTable">
                  <a-select v-model="editForm.relationTreeTable" placeholder="请选择关联树形数据表" allow-search allow-clear>
                    <a-option :value="0" label="请选择"></a-option>
                    <a-option v-for="item in treeTableOptions" :key="item.id" :label="item.name" :value="item.id"></a-option>
                  </a-select>
                  <template #extra>
              ...
              <a-col :span="12" v-if="editForm.isRelationTree === 1">
                <a-form-item label="关联字段" field="relationField">
                  <a-select v-model="editForm.relationField" placeholder="请选择关联字段" allow-search allow-clear>
                    <a-option :value="0" label="请选择"></a-option>
                    <a-option
                      v-for="item in editForm.sysGenFields"
                      :key="item.id"
                      :label="item.customName || item.dataName"
                      :value="item.id"
                    ></a-option>
                  </a-select>
                  <template #extra> 关联字段需要与关联树形数据表的主键字段类型一致。 </template>
                </a-form-item>
              </a-col>
              <a-col :span="24">
            ...
            >
              <template #columns>
                ...
                <a-table-column title="必填" :width="50" align="center">
                  <template #cell="{ record }">
                    <a-checkbox :model-value="Boolean(record.require)" @update:model-value="record.require = $event ? 1 : 0" />
                  </template>
                </a-table-column>
                <a-table-column title="列表" :width="50" align="center">
                  <template #cell="{ record }">
                    <a-checkbox :model-value="Boolean(record.listShow)" @update:model-value="record.listShow = $event ? 1 : 0" />
                  </template>
                </a-table-column>
                <a-table-column title="表单" :width="50" align="center">
                  <template #cell="{ record }">
                    <a-checkbox :model-value="Boolean(record.formShow)" @update:model-value="record.formShow = $event ? 1 : 0" />
                  </template>
                </a-table-column>
                <a-table-column title="查询" :width="50" align="center">
                  <template #cell="{ record }">
                    <a-checkbox
                      :model-value="Boolean(record.queryShow)"
                ...
                <a-table-column title="查询方式" :width="150">
                  <template #cell="{ record }">
                    <a-select v-model="record.queryType" placeholder="请选择查询方式" allow-search allow-clear>
                      <!-- 等于 -->
                ...
                <a-table-column title="表单类型" :width="150">
                  <template #cell="{ record }">
                    <a-select v-model="record.formType" placeholder="请选择表单类型" allow-search allow-clear>
                ...
                <a-table-column title="关联字典" :width="150">
                  <template #cell="{ record }">
                    <a-select v-model="record.dictType" placeholder="请选择字典类型" allow-search allow-clear>
                      <a-option v-for="item in dictOption" :key="item.value" :label="item.label" :value="item.value"></a-option>
                    </a-select>
                  </template>
                </a-table-column>
              </template>
...
import { getSysGenByIdAPI, updateSysGenAPI, getSysGenListAPI, type SysGenItem, type SysGenListParams } from "@/api/sysgen";
...
const loadTreeTableOptions = async () => {
  try {
    ...
    if (res.data?.list) {
      treeTableOptions.value = res.data.list.map((item: SysGenItem) => ({
        id: item.id,
        name: item.name
      }));
...
const fieldTableScroll = computed(() => ({
  x: "100%",
  ...(editForm.value.sysGenFields.length > 0 ? { y: 600 } : {})
}));
