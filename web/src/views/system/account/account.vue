        ...
        <template #sider>
          <div class="left-box uvp-tree-panel">
            ...
            <div class="tree-box uvp-tree-panel__body">
              <a-tree ref="treeRef" :field-names="fieldNames" :data="filteredTreeData" show-line @select="onSelectTree">
            ...
            <s-layout-search class="account-search-panel">
              <template #fields>
                ...
                <a-input
                  ...
                  @press-enter="search"
                />
                <a-select placeholder="用户状态" v-model="form.status" style="width: 126px" allow-clear>
            ...
            >
              <template #columns>
                ...
                <a-table-column title="部门" data-index="deptId" :width="150">
                  <template #cell="{ record }">{{ record.department ? record.department.name : "" }}</template>
                </a-table-column>
                <a-table-column title="角色" data-index="roles" :width="200">
                  <template #cell="{ record }">
                    <a-space wrap v-if="record.roles && record.roles.length">
                      <a-tag v-for="role in record.roles" :key="role.id" bordered size="small" color="arcoblue">
                        {{ role.name }}
                      </a-tag>
                    </a-space>
                    <span v-else>-</span>
                  ...
                  <template #cell="{ record }">
                    <div class="uvp-table-actions">
                      <a-link
                        ...
                        v-hasPerm="['system:account:edit']"
                      >
                        <template #icon><icon-edit /></template>
                        <span>修改</span>
                      </a-link>
                      <a-popconfirm type="warning" content="确定删除该账号吗?" @ok="onDelete(record)">
                        <a-link class="uvp-table-action uvp-table-action--delete" v-hasPerm="['system:account:delete']">
            ...
            <a-col :span="12">
              <a-form-item field="password" label="密码" validate-trigger="blur">
                <a-input-password
                  ...
                  :placeholder="formType == 0 ? '请输入密码' : '输入新密码，留空则不修改'"
                  allow-clear
                />
              </a-form-item>
            </a-col>
