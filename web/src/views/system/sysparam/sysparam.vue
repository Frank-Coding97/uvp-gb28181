<template>
  <div class="snow-page">
    <div class="snow-inner">
      <s-layout-search>
        <template #fields>
          <a-input v-model="form.name" placeholder="请输入参数名称" style="width: 176px" allow-clear @press-enter="search" />
          <a-input v-model="form.code" placeholder="请输入参数标识" style="width: 176px" allow-clear @press-enter="search" />
      ...
      >
        <template #columns>
          ...
          <a-table-column title="参数值" data-index="value" :ellipsis="true" :width="200" :tooltip="true"></a-table-column>
            ...
            <template #cell="{ record }">
              <div class="uvp-table-actions">
                <a-link
                  ...
                  v-hasPerm="['system:param:edit']"
                >
                  <template #icon><icon-edit /></template>
                  <span>修改</span>
                </a-link>
                <a-popconfirm type="warning" content="确定删除该参数吗?" @ok="onDelete(record)">
                  <a-link class="uvp-table-action uvp-table-action--delete" v-hasPerm="['system:param:delete']">
...
const tableScroll = computed(() => ({
  x: "100%",
  minWidth: 1000,
  ...(paramList.value.length > 0 ? { y: "100%" } : {})
}));
