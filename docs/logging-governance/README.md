...
| 等级分布 | **Warn 172（49%）** > Info 129 > Error 23 > Debug 27 | 见 C03 后记（原「Warn < 10%」已撤销；**比例不是指标**） |
| 业务链路 Warn 占比 | **53%**（业务域 248 条里 132 条 WARN） | 见 C03 后记 |
...
| 唯一 event 值 | **333 个**（269 → 272 → 287 → 330 → 331 → **333**：C03.③ 把 881 处调用共用的失败出口按状态码拆名，C01 再拆 2 个"一名多义"） | 全部进目录 |
| 登记册门禁（C09） | **0 findings**（333 事件 / 160 字段 / 1 条带理由的例外） | 保持 0（**不是**"基线为 0 才正常"，见 C09 后记） |
| 事件等级唯一性（C03） | **0 findings**（333 个事件里没有一个跨等级） | 保持 0 |
