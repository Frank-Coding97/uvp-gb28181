...
精准云台控制与精准状态查询、看守位（含设备侧自动归位）、巡航轨迹列表/详情、**辅助控制（仅雨刷，编号 1 —— 2026-09-21 收敛，见 `gb28181-2022-master-backlog.md:106`）**、
...
- 厂商私有扩展不计入标准功能。⚠️ 本条此前的举例「海康/大华的辅助控制编号事实标准」**已撤回**（2026-09-21）：
  该「2=红外灯 / 3=加热 / 4=除雾 / 5=制冷」的编号表**没有任何可用证据**——海康官方 `NET_DVR_PTZControl`
  是另一套且顺序不同（`2=LIGHT_PWRON` `3=WIPER_PWRON` `4=FAN_PWRON` `5=HEATER_PWRON` `6,7=AUX_PWRON1,2`），
  公开资料里 GB28181 辅助开关的举例**只有雨刷**。标准在 A.3.7（表 A.11）只给了唯一一个编号语义
  （「字节5为辅助开关编号，取值为"1"表示雨刷控制。」），编号域是开放值 `00H`~`FFH`，其余编号属**标准留白**，
  既不能称"事实标准"也不能称"不合规"。两侧已据此收敛到只做编号 1（见 `gb28181-2022-master-backlog.md:106`）。
