...
| 目标跟踪 `TargetTrack` | ✅ 已实现（2026-09-21）—— 意图落库（`gb_device_target_track`）+ 同路径按方法分档权限 + 画布框选换算 | ✅ 已实现（2026-09-21）—— 跟踪态 + 画布跟踪框 + 顶部头条 | ☑ D-1 | A.2.3.1.14 |
...
| 辅助控制（**仅雨刷**） | ✅ 已实现（2026-09-21）—— 独立路由 `POST /ptz/wiper`（`controlPtzWiper`）+ 编号在**后端**固定为 1 + 状态词只说「已下发」（附录 A 无回读手段）；`/ptz/extended` 继续拒绝通用 `aux_on`/`aux_off` | ✅ 已实现（2026-09-21）—— 枚举收敛为 `Wiper(1)`，「模拟中心 → 辅助功能」只剩雨刷一张卡 | — | A.3.7 / 表 A.11 |
...
| 编号 | 任务（条款） | 平台侧（后端 · 前端入口） | 模拟器侧 | 验收闭环 | P |
| --- | --- | --- | --- | --- | --- |
| ☑ D-1 | **目标跟踪 `TargetTrack`**（A.2.3.1.14）✅ **2026-09-21 完成（平台 + 模拟器双侧）**<br>⛔⛔ **本轮主结论:这也是「无应答命令」，而且比 D-2 更彻底 —— 连"事后再查一次"都没有**。标准证据:① **9.3.1 d)**（OCR `sec93.txt:25-26`）原文「源设备向目标设备发送摄像机云台控制、远程启动、强制关键帧、拉框放大、拉框缩小、PTZ 精准控制、存储卡格式化、**目标跟踪**命令后，**目标设备不发送应答命令**，命令流程见 9.3.2.1」;② **表 1 序号 13**「目标跟踪 / A.2.3.1.14 / **（无）**」。⇒ `ResponseRequired:false` + `MaxAttempts:1`。⛔ 设成 `true` 的后果同 D-2:**把一个成功当失败报**。<br>⭐ **比 D-2 更狠的一点**:存储卡格式化至少还能"事后再查一次 `SDCardStatus`"做对账，目标跟踪**附录 A 没有任何查询/上报命令** ⇒ 「设备现在在跟踪什么」**在协议上不可回答**，只有"**平台让它跟踪什么**"可答。落库表 `gb_device_target_track` 的语义因此被钉死为**平台意图**（`device_acknowledged` 恒 `false`），界面措辞只允许「已下发，设备未回执」，**不允许**「正在跟踪 / 已完成」。<br>⭐ **报文三条口径**（A.2.3.1.14，标准页 77-78）:① SN 之后的 `DeviceID` 必选 = **全景相机的球机通道**，`DeviceID2` 可选 = **全景通道**，两者不是一回事;② **比例换算由设备做** —— 正文「由于平台与设备画面比例大小不同，需要进行比例关系转化。**因此，平台应提供画面大小**:播放窗口长度像素值和播放窗口宽度像素值」⇒ 平台只如实发「渲染像素尺寸 + 该坐标系里的框选坐标」，**不得**自作主张乘宽高比;③ `Auto` 是"设备自己找目标，无需下发坐标"。<br>⛔ **前端换算基准与遮挡刻意不同**:遮挡用"设备 OSD 声明的图像尺寸"（真机 704×576，与页面无关），目标跟踪用**画面渲染像素**（`getBoundingClientRect()`）—— 框是操作员在**这块画面**上圈出来的，天然长在渲染坐标里。两套基准不能互相套用。<br>⭐ **权限照 `video-params` 先例「同路径按方法分档」**（读 `gb28181:ptz:view` / 写 `gb28181:ptz:control`），**刻意不新造独立权限码** —— 与 D-2 相反:目标跟踪是**可逆**的普通控制（不像格式化会清数据），不该比云台控制更严。<br>⛔ **四个「画面上拖」模式必须互斥**（拉框变焦 / 遮挡框选 / OSD 调位置 / 目标跟踪），`canvasModeArmed` 纳入 `targetTrackMode` 后 Esc 归属才不会打架（Arco 的全局 Esc 监听是**并行**的、随后才跑，故 layer 监听用 capture + `stopPropagation()`）。<br>⛔ 两个易漏坑:① 路由路径**两常量派生**（`TargetTrackRoutePath` 相对 + `TargetTrackAPIPath` 全路径），手写两处错一个字符的表现是「接口通但恒 403」**且两边都不报错**;② 浮点容差 `RATIO_EPSILON = 1e-9` —— `0.2 + 0.02 - 0.2` 在 IEEE754 下是 `0.01999999999999999`，不加容差会**随机**拒掉"刚好卡阈值"的框。 | ✅ 后端: `manscdp/device_advanced.go`（`TargetTrackCommand`/`TargetTrackArea`/`ParseTargetTrackMode`/`ValidateTargetTrackCommand`/`BuildTargetTrackControl[WithProfile]`）+ `ptz/target_track.go`（`ResponseRequired:false` + 意图 CAS 落库）+ 控制器 `device_target_track.go`（同路径分档 + 写侧 `requireMaintenanceAuthentication`）+ 模型 `gb_target_track.go` + 三方言 up/down 迁移 + 三全量快照（`8a381926`）<br>✅ 前端: `targetTrackBox.ts`（纯函数换算 + 单测,最小框 `0.02` / 容差 / 越界钳位不拒发 / 先浮点后统一取整）+ `PlayConsoleLinked.vue` 第四个画面模式（琥珀色覆盖层 + 三按钮 + 意图/状态/提示/错误行）+ `api/gb28181.ts`（`targetTrack?: ControlCapability`，默认 `unknown` 是**正确结果**:需要"全景 + 球机"双目结构，后端只认显式声明） | ✅ 模拟器（`uvp-gb28181-sim`）: 原先只写 `lastCommand`（UI 只认 3 秒内）⇒ **平台点完两侧都毫无动静**，且因为无回执无处排障。本轮落 **持续跟踪态** `TargetTrackState`（`domain/TargetTrack.kt`:归一化框 + 比值换算 + `SIMULATED_AUTO_BOX`），`Stop` = **置 null**;画布加**持续显示**的跟踪框（矩形 + 四角取景框角标，**不淡出** —— 拉框放大那种"200ms 入 / 1.4× 放大 / 600ms 淡出"是"一次性动作"的形态，跟踪是**持续状态**，跟着淡出会让"正在跟踪"2 秒后自己消失）;顶部 `StatusHeadline` 加分支。<br>⭐ **`Auto` 的框必须自曝「模拟目标」**:设备 AI 没接真源，框是写死的确定性常量（掷骰子没法做回归/截图）;`Manual` 的框是平台报文**真值**，不带"模拟"字样。<br>⭐ **`Manual` 无可用框时 `box = null`，绝不回落成模拟框** —— 回落会把"平台没框选"画成"框选成功"，而双方都没有回执可对。<br>⭐ **协议侧不发应答这条从"顺带成立"变成显式契约**:`DeviceControlSubRouter.handleDeviceControl` 取不到 `<RecordCmd>` 就 return ⇒ 天然不回;新增用例同时断言"确实处理了"+"没有 `<CmdType>DeviceControl</CmdType>` 出栈"。<br>⭐ **跨仓契约锚点**:把平台 `BuildTargetTrackControlWithProfile` 跑出的三段真实报文（Manual 全字段 / Auto / Stop）照抄进模拟器测试 `TargetTrackPlatformPayloadContractTest` —— 两仓各自的绿测试**管不到**"平台发的东西模拟器认不认"这条缝（元素名漂移在两仓都能全绿）。<br>⛔ **不进存档**:真机断电重启后跟踪算法不会接着盯上一条指令的目标;更要紧的是**平台没有任何手段能发现**自己看到的是重启前的旧状态（无回执 + 无查询）。 | 授权用户框选 → 平台下发 → **设备屏幕出现跟踪框并持续显示** → 再点停止 → 框消失 ✅（双侧单测 + 跨仓契约）<br>⛔ **待回填（三条，均为"没有设备"所致）**:① **真机验证未做** —— 依赖设备 AI + 双目结构，手上无对应机型;本条的 `ResponseRequired:false` **目前只有标准原文支撑**，尚无 D-2 那样的真机反证。② **模拟器实机点击未走** —— `composeApp` 无 JVM/桌面目标，本机跑不起来 Android/iOS，报文→状态这一跳已由跨仓契约覆盖，**画布渲染**仍只在单测层验证。③ **`Auto` 的框是写死的常量**，不代表任何真实目标分布。 | P2 |
...
| **目标跟踪**（A.2.3.1.14） | `manscdp/device_advanced.go`、`ptz/target_track.go`、`gb_device_target_track`、`controllers/device_target_track.go`（8 + 路由 3 单测）、前端 `targetTrackBox.ts` + `PlayConsoleLinked.vue` | ✅ `domain/TargetTrack.kt` + `TargetTrackOverlay`（画布持续框）+ `StatusHeadline` 分支 + `TargetTrackPlatformPayloadContractTest`（跨仓报文锚点） | 回归（D-1 已完成 2026-09-21）—— ⛔ **三条要各跑一遍**：① 下发后**设备屏幕出现跟踪框且不淡出**;② `Stop` 后框**消失**（不是变成"停止中"）;③ `Auto` 角标**必须带「模拟目标」**字样（去掉就等于把假目标当设备 AI 输出，而这条命令无回执可证伪）。⚠️ **真机验证尚未做**，见 D-1 行 |
...
**`D-1` 已于 2026-09-21 完成**（目标跟踪，平台 + 模拟器双侧；见 §4.D —— 与 `D-2` 同属「无应答命令」，
比 D-2 更彻底的是**连事后查询都没有**）；
...
| `docs/target-track-design.md` | 目标跟踪（D-1）端到端设计与验收：标准原文两处依据 + 「平台意图 vs 设备状态」的分野 + 三方言表结构 + 前端换算基准（与遮挡**刻意不同**）+ 模拟器屏幕可见态 + 跨仓契约锚点 + **未完成项（无真机）** |
